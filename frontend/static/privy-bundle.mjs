...
function formatAbiParameter(abiParameter) {
  ...
  if (tupleRegex.test(abiParameter.type) && "components" in abiParameter) {
    ...
    for (let i22 = 0; i22 < length; i22++) {
      const component = abiParameter.components[i22];
...
function formatAbiParameters(abiParameters) {
  ...
  for (let i22 = 0; i22 < length; i22++) {
    const abiParameter = abiParameters[i22];
...
function parseFunctionSignature(signature, structs = {}) {
  ...
  for (let i22 = 0; i22 < inputLength; i22++) {
    inputs.push(parseAbiParameter(inputParams[i22], {
  ...
  if (match.returns) {
    ...
    for (let i22 = 0; i22 < outputLength; i22++) {
      outputs.push(parseAbiParameter(outputParams[i22], {
        modifiers: functionModifiers,
        structs,
        type: "function"
      }));
...
function parseEventSignature(signature, structs = {}) {
  ...
  for (let i22 = 0; i22 < length; i22++)
    abiParameters.push(parseAbiParameter(params[i22], {
...
function parseErrorSignature(signature, structs = {}) {
  ...
  const length = params.length;
  for (let i22 = 0; i22 < length; i22++)
    abiParameters.push(parseAbiParameter(params[i22], { structs, type: "error" }));
...
function parseConstructorSignature(signature, structs = {}) {
  ...
  const length = params.length;
  for (let i22 = 0; i22 < length; i22++)
    abiParameters.push(parseAbiParameter(params[i22], { structs, type: "constructor" }));
  return {
    type: "constructor",
    stateMutability: match.stateMutability ?? "nonpayable",
...
function parseAbiParameter(param, options) {
  ...
  if (isTuple) {
    ...
    const length = params.length;
    for (let i22 = 0; i22 < length; i22++) {
      components_.push(parseAbiParameter(params[i22], { structs }));
    }
...
function splitParameters(params, result = [], current = "", depth = 0) {
  ...
  for (let i22 = 0; i22 < length; i22++) {
    const char = params[i22];
    const tail = params.slice(i22 + 1);
    switch (char) {
      case ",":
...
function parseStructs(signatures) {
  ...
  for (let i22 = 0; i22 < signaturesLength; i22++) {
    const signature = signatures[i22];
  ...
  for (let i22 = 0; i22 < entriesLength; i22++) {
    const [name, parameters] = entries[i22];
...
function parseAbiItem(signature) {
  ...
  if (typeof signature === "string")
    ...
    for (let i22 = 0; i22 < length; i22++) {
      const signature_ = signature[i22];
...
function parseAbiParameters(params) {
  ...
  if (typeof params === "string") {
    ...
    for (let i22 = 0; i22 < length; i22++) {
      abiParameters.push(parseAbiParameter(parameters[i22], { modifiers }));
    }
  } else {
    const structs = parseStructs(params);
    const length = params.length;
    for (let i22 = 0; i22 < length; i22++) {
      const signature = params[i22];
      if (isStructSignature(signature))
        continue;
...
function padBytes(bytes, { dir, size: size5 = 32 } = {}) {
  ...
  for (let i22 = 0; i22 < size5; i22++) {
    const padEnd = dir === "right";
    paddedBytes[padEnd ? i22 : size5 - i22 - 1] = bytes[padEnd ? i22 : bytes.length - i22 - 1];
  }
  return paddedBytes;
}
var init_pad = __esm({
...
function trim(hexOrBytes, { dir = "left" } = {}) {
  ...
  let sliceLength = 0;
  for (let i22 = 0; i22 < data.length - 1; i22++) {
    if (data[dir === "left" ? i22 : data.length - i22 - 1].toString() === "0")
      sliceLength++;
    else
      break;
  }
...
var init_toHex = __esm({
  "node_modules/viem/_esm/utils/encoding/toHex.js"() {
    ...
    init_pad();
    init_fromHex();
    hexes = /* @__PURE__ */ Array.from({ length: 256 }, (_v, i22) => i22.toString(16).padStart(2, "0"));
...
function split(lst, le = false) {
  ...
  for (let i22 = 0; i22 < len; i22++) {
    const { h: h10, l: l7 } = fromBig(lst[i22], le);
    [Ah[i22], Al[i22]] = [h10, l7];
  }
  return [Ah, Al];
}
...
function anumber(n9) {
  if (!Number.isSafeInteger(n9) || n9 < 0)
    throw new Error("positive integer expected, got " + n9);
}
...
function concatBytes(...arrays) {
  let sum = 0;
  for (let i22 = 0; i22 < arrays.length; i22++) {
    const a20 = arrays[i22];
    abytes(a20);
    sum += a20.length;
  }
  const res = new Uint8Array(sum);
  for (let i22 = 0, pad4 = 0; i22 < arrays.length; i22++) {
    const a20 = arrays[i22];
    res.set(a20, pad4);
    pad4 += a20.length;
  }
...
function keccakP(s11, rounds = 24) {
  ...
  for (let round = 24 - rounds; round < 24; round++) {
    ...
    let curH = s11[2];
    let curL = s11[3];
    for (let t43 = 0; t43 < 24; t43++) {
      const shift = SHA3_ROTL[t43];
      const Th = rotlH(curH, curL, shift);
      const Tl = rotlL(curH, curL, shift);
      const PI = SHA3_PI[t43];
      curH = s11[PI];
      curL = s11[PI + 1];
      s11[PI] = Th;
      s11[PI + 1] = Tl;
  ...
  "node_modules/@noble/hashes/esm/sha3.js"() {
    ...
    for (let round = 0, R = _1n, x = 1, y2 = 0; round < 24; round++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if (R & _2n)
          t43 ^= _1n << (_1n << /* @__PURE__ */ BigInt(j)) - _1n;
      }
      _SHA3_IOTA.push(t43);
    }
...
function normalizeSignature(signature) {
  ...
  let result = "";
  let valid = false;
  for (let i22 = 0; i22 < signature.length; i22++) {
    const char = signature[i22];
    if (["(", ")", ","].includes(char))
      active = true;
    if (char === "(")
      level++;
    ...
    if (char === " ") {
      if (signature[i22 - 1] !== "," && current !== "," && current !== ",(") {
        current = "";
        active = false;
      }
      continue;
...
function checksumAddress(address_, chainId) {
  ...
  for (let i22 = 0; i22 < 40; i22 += 2) {
    if (hash3[i22 >> 1] >> 4 >= 8 && address[i22]) {
      address[i22] = address[i22].toUpperCase();
    }
    if ((hash3[i22 >> 1] & 15) >= 8 && address[i22 + 1]) {
      address[i22 + 1] = address[i22 + 1].toUpperCase();
    }
  }
...
function prepareParams({ params, values }) {
  ...
  for (let i22 = 0; i22 < params.length; i22++) {
    preparedParams.push(prepareParam({ param: params[i22], value: values[i22] }));
  }
...
function encodeParams(preparedParams) {
  let staticSize = 0;
  for (let i22 = 0; i22 < preparedParams.length; i22++) {
    const { dynamic, encoded } = preparedParams[i22];
    if (dynamic)
      staticSize += 32;
    else
...
function encodeArray(value, { length, param }) {
  ...
  const preparedParams = [];
  for (let i22 = 0; i22 < value.length; i22++) {
    const preparedParam = prepareParam({ param, value: value[i22] });
...
function encodeString(value) {
  ...
  const parts = [];
  for (let i22 = 0; i22 < partsLength; i22++) {
    parts.push(padHex(slice(hexValue, i22 * 32, (i22 + 1) * 32), {
      dir: "right"
    }));
  }
  return {
...
function encodeTuple(value, { param }) {
  let dynamic = false;
  const preparedParams = [];
  for (let i22 = 0; i22 < param.components.length; i22++) {
    const param_ = param.components[i22];
    const index2 = Array.isArray(value) ? i22 : param_.name;
...
function decodeArray(cursor, param, { length, staticPosition }) {
  if (!length) {
    ...
    for (let i22 = 0; i22 < length2; ++i22) {
      cursor.setPosition(startOfData + (dynamicChild ? i22 * 32 : consumed2));
  ...
  if (hasDynamicChild(param)) {
    ...
    const value2 = [];
    for (let i22 = 0; i22 < length; ++i22) {
      cursor.setPosition(start + i22 * 32);
...
function decodeTuple(cursor, param, { staticPosition }) {
  ...
  if (hasDynamicChild(param)) {
    ...
    const start = staticPosition + offset;
    for (let i22 = 0; i22 < param.components.length; ++i22) {
      const component = param.components[i22];
      cursor.setPosition(start + consumed);
      const [data, consumed_] = decodeParameter(cursor, component, {
        staticPosition: start
      });
      consumed += consumed_;
      value[hasUnnamedChild ? i22 : component?.name] = data;
    }
    cursor.setPosition(staticPosition + 32);
    return [value, 32];
  }
...
function formatAbiItemWithArgs({ abiItem, args, includeFunctionName = true, includeName = false }) {
  ...
  if (!abiItem.inputs)
    return;
  return `${includeFunctionName ? abiItem.name : ""}(${abiItem.inputs.map((input, i22) => `${includeName && input.name ? `${input.name}: ` : ""}${typeof args[i22] === "object" ? stringify(args[i22]) : args[i22]}`).join(", ")})`;
}
    ...
    HashMD = class extends Hash {
      ...
      digestInto(out) {
        ...
        for (let i22 = 0; i22 < outLen; i22++)
          oview.setUint32(4 * i22, state[i22], isLE2);
      }
      digest() {
...
var init_sha2 = __esm({
  "node_modules/@noble/hashes/esm/sha2.js"() {
    ...
    SHA256 = class extends HashMD {
      ...
      process(view, offset) {
        for (let i22 = 0; i22 < 16; i22++, offset += 4)
          SHA256_W[i22] = view.getUint32(offset, false);
        for (let i22 = 16; i22 < 64; i22++) {
          const W15 = SHA256_W[i22 - 15];
          const W2 = SHA256_W[i22 - 2];
          const s0 = rotr(W15, 7) ^ rotr(W15, 18) ^ W15 >>> 3;
          const s1 = rotr(W2, 17) ^ rotr(W2, 19) ^ W2 >>> 10;
          SHA256_W[i22] = s1 + SHA256_W[i22 - 7] + s0 + SHA256_W[i22 - 16] | 0;
        }
        let { A, B, C, D, E: E2, F, G, H: H2 } = this;
        for (let i22 = 0; i22 < 64; i22++) {
          const sigma1 = rotr(E2, 6) ^ rotr(E2, 11) ^ rotr(E2, 25);
          const T1 = H2 + sigma1 + Chi(E2, F, G) + SHA256_K[i22] + SHA256_W[i22] | 0;
  ...
  "node_modules/@noble/hashes/esm/hmac.js"() {
    ...
    HMAC = class extends Hash {
      constructor(hash3, _key) {
        ...
        for (let i22 = 0; i22 < pad4.length; i22++)
          pad4[i22] ^= 54;
...
function numberToBytesBE(n9, len) {
  return hexToBytes2(n9.toString(16).padStart(len * 2, "0"));
}
function numberToBytesLE(n9, len) {
  return numberToBytesBE(n9, len).reverse();
}
function ensureBytes(title, hex, expectedLength) {
  ...
  if (typeof hex === "string") {
    try {
      ...
      throw new Error(title + " must be hex string or Uint8Array, cause: " + e40);
    }
...
function inRange(n9, min, max) {
  return isPosBig(n9) && isPosBig(min) && isPosBig(max) && min <= n9 && n9 < max;
}
function aInRange(title, n9, min, max) {
  if (!inRange(n9, min, max))
    throw new Error("expected valid " + title + ": " + min + " <= n < " + max + ", got " + n9);
}
function bitLen(n9) {
  let len;
  for (len = 0; n9 > _0n2; n9 >>= _1n2, len += 1)
    ;
  return len;
}
...
var init_utils4 = __esm({
  "node_modules/@noble/curves/esm/abstract/utils.js"() {
    ...
    hexes2 = /* @__PURE__ */ Array.from({ length: 256 }, (_4, i22) => i22.toString(16).padStart(2, "0"));
    asciis = { _0: 48, _9: 57, A: 65, F: 70, a: 97, f: 102 };
    isPosBig = (n9) => typeof n9 === "bigint" && _0n2 <= n9;
    bitMask = (n9) => (_1n2 << BigInt(n9)) - _1n2;
...
function invert(number, modulo) {
  ...
  let b = modulo;
  let x = _0n3, y2 = _1n3, u4 = _1n3, v = _0n3;
  while (a20 !== _0n3) {
    const q = b / a20;
    const r20 = b % a20;
    const m3 = x - u4 * q;
    const n9 = y2 - v * q;
    b = a20, a20 = r20, x = u4, y2 = v, u4 = m3, v = n9;
  }
  const gcd = b;
  if (gcd !== _1n3)
...
function sqrt3mod4(Fp, n9) {
  const p1div4 = (Fp.ORDER + _1n3) / _4n;
  const root = Fp.pow(n9, p1div4);
  if (!Fp.eql(Fp.sqr(root), n9))
    throw new Error("Cannot find square root");
  return root;
}
function sqrt5mod8(Fp, n9) {
  const p5div8 = (Fp.ORDER - _5n) / _8n;
  const n22 = Fp.mul(n9, _2n2);
  const v = Fp.pow(n22, p5div8);
  const nv = Fp.mul(n9, v);
  const i22 = Fp.mul(Fp.mul(nv, _2n2), v);
  const root = Fp.mul(nv, Fp.sub(i22, Fp.ONE));
  if (!Fp.eql(Fp.sqr(root), n9))
    throw new Error("Cannot find square root");
  return root;
}
function tonelliShanks(P) {
  ...
  return function tonelliSlow(Fp, n9) {
    if (Fp.is0(n9))
      return n9;
    if (FpLegendre(Fp, n9) !== 1)
      throw new Error("Cannot find square root");
    let M = S;
    let c4 = Fp.mul(Fp.ONE, cc);
    let t43 = Fp.pow(n9, Q);
    let R = Fp.pow(n9, Q1div2);
    while (!Fp.eql(t43, Fp.ONE)) {
      if (Fp.is0(t43))
        return Fp.ZERO;
      let i22 = 1;
      let t_tmp = Fp.sqr(t43);
      ...
      const exponent = _1n3 << BigInt(M - i22 - 1);
...
function FpInvertBatch(Fp, nums, passZero = false) {
  ...
  const multipliedAcc = nums.reduce((acc, num2, i22) => {
    if (Fp.is0(num2))
      return acc;
    inverted[i22] = acc;
  ...
  nums.reduceRight((acc, num2, i22) => {
    if (Fp.is0(num2))
      return acc;
    inverted[i22] = Fp.mul(acc, inverted[i22]);
    return Fp.mul(acc, num2);
  }, invertedAcc);
  return inverted;
}
function FpLegendre(Fp, n9) {
  const p1mod2 = (Fp.ORDER - _1n3) / _2n2;
  const powered = Fp.pow(n9, p1mod2);
...
function nLength(n9, nBitLength) {
  if (nBitLength !== void 0)
    anumber(nBitLength);
  const _nBitLength = nBitLength !== void 0 ? nBitLength : n9.toString(2).length;
...
function calcOffsets(n9, window2, wOpts) {
  const { windowSize, mask, maxNumber, shiftBy } = wOpts;
  let wbits = Number(n9 & mask);
  let nextN = n9 >> shiftBy;
  if (wbits > windowSize) {
    wbits -= maxNumber;
    nextN += _1n4;
  }
...
function wNAF(c4, bits) {
  return {
    ...
    unsafeLadder(elm, n9, p14 = c4.ZERO) {
      let d7 = elm;
      while (n9 > _0n4) {
        if (n9 & _1n4)
          p14 = p14.add(d7);
     ...
     */
    wNAF(W, precomputes, n9) {
      let p14 = c4.ZERO;
      let f2 = c4.BASE;
      const wo = calcWOpts(W, bits);
      for (let window2 = 0; window2 < wo.windows; window2++) {
        const { nextN, offset, isZero, isNeg, isNegF, offsetF } = calcOffsets(n9, window2, wo);
        n9 = nextN;
        if (isZero) {
     ...
     * @returns point
     */
    wNAFUnsafe(W, precomputes, n9, acc = c4.ZERO) {
      const wo = calcWOpts(W, bits);
      for (let window2 = 0; window2 < wo.windows; window2++) {
        if (n9 === _0n4)
          break;
        const { nextN, offset, isZero, isNeg } = calcOffsets(n9, window2, wo);
        n9 = nextN;
        if (isZero) {
          continue;
        } else {
    ...
    wNAFCached(P, n9, transform) {
      const W = getW(P);
      return this.wNAF(W, this.getPrecomputes(W, P, transform), n9);
    },
    wNAFCachedUnsafe(P, n9, transform, prev) {
      const W = getW(P);
      if (W === 1)
        return this.unsafeLadder(P, n9, prev);
      return this.wNAFUnsafe(W, this.getPrecomputes(W, P, transform), n9, prev);
    },
...
function weierstrassPoints(opts) {
  ...
  class Point2 {
    ...
    static fromAffine(p14) {
      const { x, y: y2 } = p14 || {};
      if (!p14 || !Fp.isValid(x) || !Fp.isValid(y2))
        throw new Error("invalid affine point");
      if (p14 instanceof Point2)
        throw new Error("projective point not allowed");
      const is0 = (i22) => Fp.eql(i22, Fp.ZERO);
      if (is0(x) && is0(y2))
        return Point2.ZERO;
      return new Point2(x, y2, Fp.ONE);
    }
    get x() {
    ...
    static normalizeZ(points) {
      ...
      return points.map((p14, i22) => p14.toAffine(toInv[i22])).map(Point2.fromAffine);
    }
    /**
    ...
    add(other) {
      ...
      let t43 = Fp.add(X2, Y2);
      t310 = Fp.mul(t310, t43);
      t43 = Fp.add(t0, t1);
      t310 = Fp.sub(t310, t43);
      t43 = Fp.add(X1, Z1);
    ...
    wNAF(n9) {
      return wnaf.wNAFCached(this, n9, Point2.normalizeZ);
    }
    /**
...
function weierstrass(curveDef) {
  ...
  class Signature {
    ...
    static fromCompact(hex) {
      ...
      return new Signature(slcNum(hex, 0, l7), slcNum(hex, l7, 2 * l7));
    }
  ...
  function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
    ...
    if (ent != null && ent !== false) {
      const e40 = ent === true ? randomBytes2(Fp.BYTES) : ent;
      seedArgs.push(ensureBytes("extraEntropy", e40));
    }
...
function SWUFpSqrtRatio(Fp, Z2) {
  const q = Fp.ORDER;
  let l7 = _0n5;
  for (let o17 = q - _1n5; o17 % _2n3 === _0n5; o17 /= _2n3)
    l7 += _1n5;
  const c1 = l7;
    ...
    DER = {
      ...
      _tlv: {
        encode: (tag, data) => {
          ...
          const t43 = numberToHexUnpadded(tag);
          return t43 + lenLen + len + data;
        },
...
function strxor(a20, b) {
  ...
  for (let i22 = 0; i22 < a20.length; i22++) {
    arr[i22] = a20[i22] ^ b[i22];
  }
  return arr;
}
function anum(item) {
...
function expand_message_xmd(msg, DST, lenInBytes, H2) {
  ...
  for (let i22 = 1; i22 <= ell; i22++) {
    const args = [strxor(b_0, b[i22 - 1]), i2osp(i22 + 1, 1), DST_prime];
    b[i22] = H2(concatBytes3(...args));
  }
...
function hash_to_field(msg, count, options) {
  ...
  for (let i22 = 0; i22 < count; i22++) {
    const e40 = new Array(m3);
    for (let j = 0; j < m3; j++) {
      const elm_offset = L * (j + i22 * m3);
      const tv = prb.subarray(elm_offset, elm_offset + L);
      e40[j] = mod(os2ip(tv), p14);
    }
    u4[i22] = e40;
  }
  return u4;
...
function isogenyMap(field, map) {
  ...
  return (x, y2) => {
    const [xn, xd, yn, yd] = coeff.map((val) => val.reduce((acc, i22) => field.add(field.mul(acc, x), i22)));
...
function schnorrSign(message, privateKey, auxRand = randomBytes(32)) {
  ...
  const t43 = numTo32b(d7 ^ num(taggedHash("BIP0340/aux", a20)));
  const rand = taggedHash("BIP0340/nonce", t43, px, m3);
  const k_ = modN(num(rand));
  if (k_ === _0n6)
    throw new Error("sign failed: k is zero");
  const { bytes: rx, scalar: k2 } = schnorrGetExtPubKey(k_);
  const e40 = challenge(rx, px, m3);
  const sig = new Uint8Array(64);
  sig.set(rx, 0);
  sig.set(numTo32b(modN(k2 + e40 * d7)), 32);
...
function schnorrVerify(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange(s11, _1n6, secp256k1N))
      return false;
    const e40 = challenge(numTo32b(r20), pointToBytes(P), m3);
    const R = GmulAdd(P, s11, modN(-e40));
...
var init_secp256k1 = __esm({
  "node_modules/@noble/curves/esm/secp256k1.js"() {
    ...
    secp256k1 = createCurve({
      ...
      endo: {
        ...
        splitScalar: (k2) => {
          ...
          const c1 = divNearest(b2 * k2, n9);
          const c22 = divNearest(-b1 * k2, n9);
          let k1 = mod(k2 - c1 * a1 - c22 * a22, n9);
          let k22 = mod(-c1 * b1 - c22 * b2, n9);
    ...
    numTo32b = (n9) => numberToBytesBE(n9, 32);
...
function getNodeError(err, args) {
  ...
  const executionRevertedError = err instanceof BaseError2 ? err.walk((e40) => e40?.code === ExecutionRevertedError.code) : err;
...
var init_Hex = __esm({
  "node_modules/ox/_esm/core/Hex.js"() {
    ...
    hexes3 = /* @__PURE__ */ Array.from({ length: 256 }, (_v, i22) => i22.toString(16).padStart(2, "0"));
  ...
  const exec = async () => {
    ...
    fn(args).then((data) => {
      if (sort && Array.isArray(data))
        data.sort(sort);
      for (let i22 = 0; i22 < scheduler.length; i22++) {
        const { resolve } = scheduler[i22];
        resolve?.([data[i22], data]);
      }
    }).catch((err) => {
      for (let i22 = 0; i22 < scheduler.length; i22++) {
        const { reject } = scheduler[i22];
        reject?.(err);
      }
    });
  };
...
async function localBatchGatewayRequest(parameters) {
  ...
  const failures = [];
  const responses = [];
  await Promise.all(queries.map(async (query, i22) => {
    try {
      responses[i22] = query.urls.includes(localBatchGatewayUrl) ? await localBatchGatewayRequest({ data: query.data, ccipRequest: ccipRequest2 }) : await ccipRequest2(query);
      failures[i22] = false;
    } catch (err) {
      failures[i22] = true;
      responses[i22] = encodeError(err);
    }
  }));
...
async function ccipRequest({ data, sender, urls }) {
  ...
  for (let i22 = 0; i22 < urls.length; i22++) {
    const url = urls[i22];
...
var require_eventemitter3 = __commonJS({
  "node_modules/eventemitter3/index.js"(exports, module) {
    ...
    EventEmitter2.prototype.listeners = function listeners(event) {
      ...
      for (var i22 = 0, l7 = handlers.length, ee = new Array(l7); i22 < l7; i22++) {
    ...
    EventEmitter2.prototype.emit = function emit(event, a1, a22, a32, a42, a52) {
      ...
      var listeners = this._events[evt], len = arguments.length, args, i22;
      if (listeners.fn) {
        ...
        for (i22 = 1, args = new Array(len - 1); i22 < len; i22++) {
        ...
        for (i22 = 0; i22 < length; i22++) {
          if (listeners[i22].once) this.removeListener(event, listeners[i22].fn, void 0, true);
          switch (len) {
            case 1:
            ...
            case 2:
              ...
              break;
            case 3:
              listeners[i22].fn.call(listeners[i22].context, a1, a22);
              break;
            case 4:
              listeners[i22].fn.call(listeners[i22].context, a1, a22, a32);
              break;
            default:
  ...
  "node_modules/canonicalize/lib/canonicalize.js"(exports, module) {
    ...
    module.exports = function serialize(object) {
      ...
      if (Array.isArray(object)) {
        const values2 = object.reduce((t43, cv, ci) => {
          const comma = ci === 0 ? "" : ",";
          const value = cv === void 0 || typeof cv === "symbol" ? null : cv;
          return `${t43}${comma}${serialize(value)}`;
        }, "");
        return `[${values2}]`;
      }
      const values = Object.keys(object).sort().reduce((t43, cv) => {
        if (object[cv] === void 0 || typeof object[cv] === "symbol") {
          return t43;
        }
        const comma = t43.length === 0 ? "" : ",";
        return `${t43}${comma}${serialize(cv)}:${serialize(object[cv])}`;
      }, "");
      return `{${values}}`;
    };
  }
...
var e6 = (e40, c4) => c4 ? Object.entries(c4).reduce(((e41, [c5, r20]) => e41.replace(`:${c5}`, `${r20}`)), e40) : e40;
...
var encodeBase64 = (input) => {
  ...
  const arr = [];
  for (let i22 = 0; i22 < unencoded.length; i22 += CHUNK_SIZE) {
    arr.push(String.fromCharCode.apply(null, unencoded.subarray(i22, i22 + CHUNK_SIZE)));
  }
...
var t13 = class _t {
  ...
  isExpired(e40 = 0) {
    return Date.now() >= 1e3 * (this.expiration - e40);
  }
  constructor(t43) {
    this.value = t43, this._decoded = decodeJwt(t43);
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/CrossAppApi.mjs
var t14 = class _t {
  async updateOnCrossAppAuthentication(r20, e40) {
    let s11 = e40.access_token, o17 = _t.providerAccessTokenStorageKey(r20);
    await this._storage.put(o17, s11);
  }
  async getProviderAccessToken(r20) {
    let s11 = _t.providerAccessTokenStorageKey(r20), o17 = await this._storage.get(s11);
    if ("string" != typeof o17) return null;
    try {
      if (new t13(o17).isExpired()) throw Error("JWT is expired");
      return o17;
    } catch {
  ...
  constructor(r20, e40) {
    this._privyInternal = r20, this._storage = e40;
  }
};
...
var Z = new Set(H.map(((i22) => i22.id)));

// node_modules/@privy-io/chains/dist/esm/ethereum/dedupe-supported-chains.mjs
var t27 = (t43) => t43.map(((t44) => {
  if (t44.rpcUrls.privyWalletOverride) return t44;
  let n9 = H.find(((i22) => i22.id === t44.id)), s11 = n9?.rpcUrls.privy?.http[0];
  return s11 ? r3(t44, s11) : t44;
}));
...
var r6 = class extends Error {
  constructor({ error: r20, code: e40, status: o17 }) {
    super(r20), this.name = "PrivyApiError", this.error = r20, this.code = e40, this.status = o17;
  }
};
var e24 = class extends Error {
  constructor({ error: r20, code: e40 }) {
    super(r20), this.code = e40, this.error = r20;
  }
};
var o8 = class extends Error {
  constructor({ error: r20, code: e40, response: o17 }) {
    super(r20), this.code = e40, this.error = r20, this.response = o17;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/wallet-api/unified-wallet.mjs
var e25 = (e40) => !!e40.id && "privy-v2" === e40.recovery_method;

// node_modules/@privy-io/js-sdk-core/dist/esm/embedded/EmbeddedBitcoinWalletProvider.mjs
var r7 = class {
  async sign({ message: e40 }) {
    return await this.request({ method: "sign", params: { message: new TextDecoder("utf8").decode(e40) } });
  }
  async signTransaction({ psbt: e40 }) {
    return await this.request({ method: "signTransaction", params: { psbt: e40 } });
  }
  async request(r20) {
  ...
  async handleIFrameRpc(t43) {
    try {
      let e40 = await this._privyInternal.getAccessTokenInternal();
      if (!e40) throw Error("Missing access token. User must be authenticated.");
      return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_started", { method: t43.method, address: this._account.address }), (await this._proxy.rpcWallet({ accessToken: e40, request: t43, entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, hdWalletIndex: this._account.wallet_index, chainType: this._account.chain_type })).response.data;
    } catch (r20) {
      console.error(r20);
      let s11 = r20 instanceof Error ? r20.message : "Unable to make wallet request";
      throw this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_failed", { method: t43.method, address: this._account.address, error: s11 }), new e24({ code: "embedded_wallet_request_error", error: s11 });
    }
  }
  toJSON() {
    return `PrivyEmbeddedBitcoinProvider { address: '${this._account.address}', request: [Function] }`;
  }
  constructor({ proxy: e40, privyInternal: t43, account: r20, entropyId: s11, entropyIdVerifier: a20 }) {
    this._proxy = e40, this._privyInternal = t43, this._account = r20, this._entropyId = s11, this._entropyIdVerifier = a20;
  }
};
...
var s6 = class extends Error {
  ...
  constructor(e40, t43, r20) {
    super(e40), t43 instanceof Error && (this.cause = t43), this.privyErrorCode = r20;
  }
};
var o9 = class extends Error {
  constructor(e40, t43) {
    super(t43), this.type = e40;
  }
};
var i9 = class extends s6 {
  constructor(e40, t43, r20) {
    super(e40, t43, r20), this.type = "connector_error";
  }
};
var d2 = class extends Error {
  constructor(e40, t43, r20) {
    super(e40), this.code = t43, this.data = r20;
  }
};
function _(t43) {
  let r20 = t43.type;
...
function E(e40) {
  return _(e40) && "wallet_not_on_device" === e40.type;
}

// node_modules/@privy-io/js-sdk-core/dist/esm/wallet-api/create.mjs
async function r9(r20, { request: t43, headers: o17 }) {
  return await r20.fetchPrivyRoute(t12, { body: t43, headers: o17 });
}

// node_modules/@privy-io/js-sdk-core/dist/esm/embedded/stack/walletCreate.mjs
async function t31({ context: t43, chainType: i22, idempotencyKey: o17 }) {
  return { wallet: await r9(t43, { request: { chain_type: i22, owner_id: void 0 }, headers: o17 ? { "privy-idempotency-key": o17 } : void 0 }) };
}
...
function encodeEventTopics(parameters) {
  ...
  if (args && "inputs" in abiItem) {
    ...
    if (args_.length > 0) {
      topics = indexedInputs?.map((param, i22) => {
        if (Array.isArray(args_[i22]))
          return args_[i22].map((_4, j) => encodeArg({ param, value: args_[i22][j] }));
        return typeof args_[i22] !== "undefined" && args_[i22] !== null ? encodeArg({ param, value: args_[i22] }) : null;
      }) ?? [];
    }
  }
...
function blobsToProofs(parameters) {
  ...
  const proofs = [];
  for (let i22 = 0; i22 < blobs.length; i22++) {
    const blob = blobs[i22];
    const commitment = commitments[i22];
...
async function prepareTransactionRequest(client, args) {
  ...
  const fillResult = attemptFill ? await getAction(client, fillTransaction, "fillTransaction")({ ...request, nonce }).then((result) => {
    ...
    if (error.name !== "TransactionExecutionError")
      return request;
    const unsupported = error.walk?.((e41) => {
      const error2 = e41;
...
function decodeEventLog(parameters) {
  ...
  const indexedInputs = inputs.map((x, i22) => [x, i22]).filter(([x]) => "indexed" in x && x.indexed);
  const missingIndexedInputs = [];
  for (let i22 = 0; i22 < indexedInputs.length; i22++) {
    const [param, argIndex] = indexedInputs[i22];
    const topic = argTopics[i22];
    if (!topic) {
      if (strict)
    ...
    if (data && data !== "0x") {
      try {
        ...
        if (decodedData) {
          ...
          if (isUnnamed) {
            for (let i22 = 0; i22 < inputs.length; i22++)
              if (args[i22] === void 0 && dataIndex < decodedData.length)
                args[i22] = decodedData[dataIndex++];
          } else
            for (let i22 = 0; i22 < nonIndexedInputs.length; i22++)
              args[nonIndexedInputs[i22].name] = decodedData[dataIndex++];
        }
      } catch (err) {
        if (strict) {
...
function isNullUniversalResolverError(err) {
  if (!(err instanceof BaseError2))
    return false;
  const cause = err.walk((e40) => e40 instanceof ContractFunctionRevertedError);
...
function namehash(name) {
  ...
  for (let i22 = labels.length - 1; i22 >= 0; i22 -= 1) {
    const hashFromEncodedLabel = encodedLabelToLabelhash(labels[i22]);
    const hashed = hashFromEncodedLabel ? toBytes(hashFromEncodedLabel) : keccak256(stringToBytes(labels[i22]), "bytes");
...
function packetToBytes(packet) {
  ...
  for (let i22 = 0; i22 < list.length; i22++) {
    let encoded = stringToBytes(list[i22]);
    if (encoded.byteLength > 255)
      encoded = stringToBytes(encodeLabelhash(labelhash(list[i22])));
...
function encodeType({ primaryType, types }) {
  ...
  for (const type of deps) {
    result += `${type}(${types[type].map(({ name, type: t43 }) => `${t43} ${name}`).join(",")})`;
  }
  return result;
}
...
function encodeField({ types, name, type, value }) {
  ...
  if (type.lastIndexOf("]") === type.length - 1) {
    ...
    return [
      { type: "bytes32" },
      keccak256(encodeAbiParameters(typeValuePairs.map(([t43]) => t43), typeValuePairs.map(([, v]) => v)))
    ];
  }
  return [{ type }, value];
}
...
function checksum2(address) {
  ...
  for (let i22 = 0; i22 < 40; i22 += 2) {
    if (hash3[i22 >> 1] >> 4 >= 8 && characters[i22]) {
      characters[i22] = characters[i22].toUpperCase();
    }
    if ((hash3[i22 >> 1] & 15) >= 8 && characters[i22 + 1]) {
      characters[i22 + 1] = characters[i22 + 1].toUpperCase();
    }
  }
...
function prepareParameters({ checksumAddress: checksumAddress2, parameters, values }) {
  ...
  for (let i22 = 0; i22 < parameters.length; i22++) {
    preparedParameters.push(prepareParameter({
      checksumAddress: checksumAddress2,
      parameter: parameters[i22],
      value: values[i22]
    }));
  }
...
function encode3(preparedParameters) {
  let staticSize = 0;
  for (let i22 = 0; i22 < preparedParameters.length; i22++) {
    const { dynamic, encoded } = preparedParameters[i22];
    if (dynamic)
      staticSize += 32;
    else
...
function encodeString2(value) {
  ...
  const parts = [];
  for (let i22 = 0; i22 < partsLength; i22++) {
    parts.push(padRight(slice3(hexValue, i22 * 32, (i22 + 1) * 32)));
  }
  return {
    dynamic: true,
...
function encodeTuple2(value, options) {
  ...
  let dynamic = false;
  const preparedParameters = [];
  for (let i22 = 0; i22 < parameter.components.length; i22++) {
    const param_ = parameter.components[i22];
    const index2 = Array.isArray(value) ? i22 : param_.name;
    const preparedParam = prepareParameter({
      checksumAddress: checksumAddress2,
      parameter: param_,
...
function encodePacked(types, values) {
  ...
  const data = [];
  for (let i22 = 0; i22 < types.length; i22++) {
    const type = types[i22];
...
(function(encodePacked2) {
  function encode6(type, value, isArray = false) {
    ...
    if (arrayMatch && Array.isArray(value)) {
      ...
      const data = [];
      for (let i22 = 0; i22 < value.length; i22++) {
        data.push(encode6(childType, value[i22], true));
      }
      if (data.length === 0)
        return "0x";
      return concat2(...data);
...
async function multicall(client, parameters) {
  ...
  for (let i22 = 0; i22 < contracts.length; i22++) {
    const { abi: abi2, address, args, functionName } = contracts[i22];
    try {
      const callData = encodeFunctionData({ abi: abi2, args, functionName });
  ...
  const results = [];
  for (let i22 = 0; i22 < aggregate3Results.length; i22++) {
    const result = aggregate3Results[i22];
    if (result.status === "rejected") {
      if (!allowFailure)
        throw result.reason;
      for (let j = 0; j < chunkedCalls[i22].length; j++) {
        results.push({
          status: "failure",
          error: result.reason,
          result: void 0
    ...
    for (let j = 0; j < aggregate3Result.length; j++) {
      ...
      const { callData } = chunkedCalls[i22][j];
...
async function simulateBlocks(client, parameters) {
  ...
  try {
    ...
    return result.map((block2, i22) => ({
      ...formatBlock(block2),
      calls: block2.calls.map((call2, j) => {
        const { abi: abi2, args, functionName, to } = blocks[i22].calls[j];
...
async function simulateCalls(client, parameters) {
  ...
  const blocks = await simulateBlocks(client, {
    ...
    blocks: [
      ...traceAssetChanges ? [
        ...
        // Asset pre balances
        {
          calls: assetAddresses.map((address, i22) => ({
            abi: [
  ...
  const changes = [];
  for (const [i22, balancePost] of balancesPost.entries()) {
    const balancePre = balancesPre[i22];
    if (typeof balancePost !== "bigint")
      continue;
    if (typeof balancePre !== "bigint")
      continue;
    const decimals_ = decimals[i22 - 1];
    const symbol_ = symbols[i22 - 1];
    const tokenURI_ = tokenURI[i22 - 1];
    const token = (() => {
      if (i22 === 0)
        return {
          address: ethAddress,
      ...
      try {
        ...
        if (prevBlockNumber !== void 0) {
          ...
          if (blockNumber - prevBlockNumber > 1 && emitMissed) {
            for (let i22 = prevBlockNumber + 1n; i22 < blockNumber; i22++) {
              emit.onBlockNumber(i22, prevBlockNumber);
              prevBlockNumber = i22;
            }
          }
        }
      ...
      try {
        ...
        if (block.number !== null && prevBlock?.number != null) {
          ...
          if (block.number - prevBlock.number > 1 && emitMissed) {
            for (let i22 = prevBlock?.number + 1n; i22 < block.number; i22++) {
...
var p11 = (p14, i22, c4, s11) => {
  let l7 = Number(p14), n9 = i22.find(((r20) => r20.id === l7));
  if (!n9) throw new i9(`Unsupported chainId ${p14}`, 4901);
  return createPublicClient({ transport: http(o10(n9, c4, s11.appId)), chain: n9 });
};
var o10 = (r20, e40, p14) => {
  let o17, i22 = r20.id, c4 = Number(r20.id);
  if (r20.rpcUrls.privyWalletOverride && r20.rpcUrls.privyWalletOverride.http[0]) o17 = r20.rpcUrls.privyWalletOverride.http[0];
  else if (e40.rpcUrls && e40.rpcUrls[c4]) o17 = e40.rpcUrls[c4];
  else if (r20.rpcUrls.privy?.http[0]) {
    let e41 = new URL(r20.rpcUrls.privy.http[0]);
    e41.searchParams.append("privyAppId", p14), o17 = e41.toString();
  } else o17 = r20.rpcUrls.public?.http[0] ? r20.rpcUrls.public.http[0] : r20.rpcUrls.default?.http[0];
  if (!o17) throw new i9(`No RPC url found for ${i22}`);
  return o17;
};
function i10({ currentRecoveryMethod: r20, upgradeToRecoveryMethod: e40 }) {
  switch (r20) {
    case "privy":
    case "user-passcode":
    ...
    case "icloud":
    case "google-drive":
      if (r20 === e40) throw Error("Cannot upgrade to the existing cloud platform");
      return true;
    default:
...
var e27 = class {
  enqueue(e40, a20) {
    this.callbacks[e40] = a20;
  }
  dequeue(e40, a20) {
    let r20 = this.callbacks[a20];
    if (!r20) throw Error(`cannot dequeue ${e40} event: no event found for id ${a20}`);
    switch (delete this.callbacks[a20], e40) {
      ...
      case "privy:mfa:clear":
        return r20;
      default:
        throw Error(`invalid wallet event type ${e40}`);
    }
  }
  constructor() {
    this.callbacks = {};
...
async function r10(r20, t43, i22, a20, n9 = false, o17, c4) {
  let m3 = n9, s11 = async (o18) => {
    if (m3) {
      o18 === (n9 ? 0 : 1) ? a20() : i22.current?.reject(new o9("missing_or_invalid_mfa", "MFA verification failed, retry."));
      let m4 = await new Promise(((r21, a21) => {
        t43.current = { resolve: r21, reject: a21 }, setTimeout((() => {
          let r22 = new o9("mfa_timeout", "Timed out waiting for MFA code");
          i22.current?.reject(r22), a21(r22);
        }), c4);
      }));
      return await r20(m4);
    }
  ...
  for (let e40 = 0; e40 < o17; e40++) try {
    f2 = await s11(e40), i22.current?.resolve(void 0);
    break;
  } catch (e41) {
    if ("missing_or_invalid_mfa" !== e41.type) throw i22.current?.resolve(void 0), e41;
    m3 = true;
  }
  if (null === f2) {
    let r21 = new o9("mfa_verification_max_attempts_reached", "Max MFA verification attempts reached");
    throw i22.current?.reject(r21), r21;
  }
  return f2;
}
...
var a16;
var s7 = (a16 = 0, () => "id-" + a16++);
var n3 = (e40, t43) => "bigint" == typeof t43 ? t43.toString() : t43;
var l2 = async (e40, { ms: t43, msg: i22 }) => {
  let r20, a20 = new Promise(((e41, a21) => {
    r20 = setTimeout((() => {
      a21(Error(i22));
    }), t43 ?? 15e3);
  }));
  try {
    return await Promise.race([e40, a20]);
  } finally {
...
var m = new e27();
var h8 = class {
  invokeWithMfa(e40, t43) {
    return l2(r10(((i22) => l2(this.waitForReady().then((() => e40(i22))), { msg: t43.timeoutMsg, ms: t43.timeoutMs })), this.mfa.rootPromise, this.mfa.submitPromise, (() => this.mfa.emit("mfaRequired")), t43.mfaAlwaysRequired ?? false, 4, 3e5), { msg: "Operation reached timeout: MFA verification", ms: 126e4 });
  }
  reload() {
    return this.ready = false, this._embeddedWalletMessagePoster.reload();
  }
  ping(e40 = 15e3) {
    return l2(this.invoke("privy:iframe:ready", {}), { msg: "Ping reached timeout", ms: e40 });
  }
  create(e40) {
    return l2(this.waitForReady().then((() => this.invoke("privy:wallet:create", e40))), { msg: "Operation reached timeout: create" });
  }
  rpc(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:wallet:rpc", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: rpc" });
  }
  createSolana(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:solana-wallet:create", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: create", timeoutMs: 6e4 });
  }
  createAdditionalSolana(e40) {
    return l2(this.waitForReady().then((() => this.invoke("privy:solana-wallet:create-additional", e40))), { msg: "Operation reached timeout: create" });
  }
  solanaRpc(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:solana-wallet:rpc", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: solana-rpc" });
  }
  delegateWallets(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:delegated-actions:consent", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: delegated-actions:consent" });
  }
  verifyMfa(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:mfa:verify", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: mfa:verify", mfaAlwaysRequired: true });
  }
  initEnrollMfa(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:mfa:init-enrollment", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: mfa:init-enrollment" });
  }
  submitEnrollMfa(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:mfa:submit-enrollment", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: mfa:submit-enrollment" });
  }
  unenrollMfa(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:mfa:unenroll", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: mfa:unenroll", mfaAlwaysRequired: true });
  }
  clearMfa(e40) {
    return l2(this.waitForReady().then((() => this.invoke("privy:mfa:clear", e40))), { msg: "Operation reached timeout: mfa:clear" });
  }
  createWallet(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:wallets:create", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: create", timeoutMs: 6e4 });
  }
  signWithUserSigner(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:user-signer:sign", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: user-signer:sign" });
  }
  addWallet(e40) {
    return l2(this.waitForReady().then((() => this.invoke("privy:wallets:add", e40))), { msg: "Operation reached timeout: wallets:add" });
  }
  setRecovery(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:wallets:set-recovery", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: wallets:set-recovery", timeoutMs: 6e4 });
  }
  connect(e40) {
    return l2(this.waitForReady().then((() => this.invoke("privy:wallets:connect", e40))), { msg: "Operation reached timeout: wallets:connect" });
  }
  recover(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:wallets:recover", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: wallets:recover", timeoutMs: 6e4 });
  }
  rpcWallet(e40) {
    return this.invokeWithMfa(((t43) => this.invoke("privy:wallets:rpc", { ...t43, ...e40 })), { timeoutMsg: "Operation reached timeout: wallets:rpc" });
  }
  handleEmbeddedWalletMessages(e40) {
    if (!e40.event.startsWith("privy:")) return void console.warn(`Unsupported event type: ${e40.event}`);
    let { reject: t43, resolve: r20 } = m.dequeue(e40.event, e40.id);
    return void 0 !== e40.error ? t43(new o9(e40.error.type, e40.error.message)) : r20(e40.data);
  }
  waitForReady() {
    return this.ready ? Promise.resolve() : new Promise((async (t43, i22) => {
      for (; !this.ready; ) this.invoke("privy:iframe:ready", {}).then((() => {
        this.ready = true, t43();
      })).catch(i22), await e26(150);
    }));
  }
  invoke(e40, t43) {
    let i22 = ((e41, t44) => `${e41}${JSON.stringify(t44, n3)}`)(e40, t43);
    if ("privy:wallet:create" === e40 || "privy:solana-wallet:create" === e40) {
      let e41 = this.cache.get(i22);
      if (e41) return e41;
    }
    let r20 = new Promise(((i23, r21) => {
      let a20 = s7();
      m.enqueue(a20, { resolve: i23, reject: r21 }), this._embeddedWalletMessagePoster.postMessage(JSON.stringify({ id: a20, event: e40, data: t43 }), "*");
    })).finally((() => {
      this.cache.delete(i22);
    }));
    return this.cache.set(i22, r20), r20;
  }
  constructor(e40, t43) {
    this.ready = false, this.cache = /* @__PURE__ */ new Map(), this._embeddedWalletMessagePoster = e40, this.mfa = t43;
  }
};
...
var s8 = (e40) => void 0 !== e40 ? BigInt(e40) : void 0;
function t32(i22) {
  let t43, { type: c4 = 2, ...o17 } = "string" == typeof i22 ? JSON.parse(i22) : i22;
  o17.accessList && Array.isArray(o17.accessList) ? t43 = o17.accessList.map(((e40) => Array.isArray(e40) ? { address: e40[0], storageKeys: e40[1] } : e40)) : o17.accessList && (t43 = Object.entries(o17.accessList).map(((e40) => ({ address: e40[0], storageKeys: e40[1] }))));
  let d7 = Number(o17.chainId ?? 1), n9 = isHex(o17.data) ? o17.data : o17.data ? toHex(Uint8Array.from(o17.data)) : void 0, p14 = o17.nonce ? Number(o17.nonce) : void 0, m3 = { chainId: d7, data: n9, nonce: p14, value: s8(o17.value), gas: s8(o17.gas ?? o17.gasLimit) }, y2 = Number(c4);
  if (0 === y2) return { ...o17, type: r11[y2], ...m3, gasPrice: s8(o17.gasPrice), accessList: void 0, maxFeePerGas: void 0, maxPriorityFeePerGas: void 0 };
  if (1 === y2) return { ...o17, type: r11[y2], ...m3, gasPrice: s8(o17.gasPrice), accessList: t43, maxFeePerGas: void 0, maxPriorityFeePerGas: void 0 };
  if (2 === y2) return { ...o17, type: r11[y2], ...m3, nonce: p14, accessList: t43, maxFeePerGas: s8(o17.maxFeePerGas), maxPriorityFeePerGas: s8(o17.maxPriorityFeePerGas), gasPrice: void 0, maxFeePerBlobGas: void 0 };
  throw Error(`Unsupported transaction type: ${c4}`);
}

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/encodings.mjs
var t33 = (t43) => /^0x[0-9a-fA-F]*$/.test(t43);
var f = (t43) => Buffer.from(t43, "utf8");
var e28 = (t43) => `0x${t43.toString("hex")}`;
...
async function i12(i22, o17) {
  let t43 = (0, import_canonicalize.default)(o17);
  if (void 0 === t43) throw new e24({ error: "Failed to prepare the payload for signing", code: "invalid_input" });
  let a20 = Buffer.from(t43).toString("base64"), { signature: n9 } = await i22({ message: a20 });
  return { signature: n9 };
}

// node_modules/@privy-io/js-sdk-core/dist/esm/wallet-api/types.mjs
var e29 = "privy-request-expiry";
var r13 = 18e5;

// node_modules/@privy-io/js-sdk-core/dist/esm/wallet-api/rpc.mjs
var import_canonicalize2 = __toESM(require_canonicalize(), 1);
async function e30(e40, o17, { wallet_id: p14, ...m3 }) {
  let n9 = e40.getCompiledPath(p9, { params: { wallet_id: p14 } }), s11 = String(Date.now() + r13), d7 = { version: 1, url: n9, method: p9.method, headers: { "privy-app-id": e40.app.appId, [e29]: s11 }, body: { ...m3 } }, { signature: l7 } = await i12(o17, d7);
  return await e40.fetchPrivyRoute(p9, { body: m3, params: { wallet_id: p14 }, headers: { "privy-authorization-signature": l7, [e29]: s11 } });
}

// node_modules/@privy-io/js-sdk-core/dist/esm/embedded/stack/wallet-api-eth-transaction.mjs
function t34(t43) {
  return { from: t43.from, to: t43.to ?? void 0, nonce: n4(t43.nonce), chain_id: n4(t43.chainId), data: (function(t44) {
    if (void 0 !== t44) return "string" == typeof t44 ? t33(t44) ? t44 : e28(f(t44)) : e28(Buffer.from(Uint8Array.from(t44)));
  })(t43.data), value: n4(t43.value), type: t43.type, gas_limit: n4(t43.gasLimit), gas_price: n4(t43.gasPrice), max_fee_per_gas: n4(t43.maxFeePerGas), max_priority_fee_per_gas: n4(t43.maxPriorityFeePerGas) };
}
function n4(t43) {
  if ("number" == typeof t43 || "bigint" == typeof t43) {
    return `0x${BigInt(t43).toString(16)}`;
  }
  if ("string" == typeof t43) return t33(t43) ? t43 : e28(f(t43));
}

// node_modules/@privy-io/js-sdk-core/dist/esm/embedded/stack/wallet-api-eth-typed-data.mjs
function e31(e40) {
  return "string" == typeof e40 && (e40 = JSON.parse(e40)), { types: e40.types, primary_type: String(e40.primaryType), domain: e40.domain, message: e40.message };
}
...
async function i13({ context: i22, account: o17, rpcRequest: c4 }) {
  switch (c4.chainType) {
    case "ethereum":
      return (async function({ context: i23, account: o18, rpcRequest: c5 }) {
        switch (c5.method) {
          case "personal_sign": {
            let [e40] = c5.params, t43 = await e30(i23, i23.signRequest, { chain_type: "ethereum", method: "personal_sign", wallet_id: o18.id, params: e40.startsWith("0x") ? { message: e40.slice(2), encoding: "hex" } : { message: e40, encoding: "utf-8" } });
            if ("personal_sign" !== t43.method) throw Error("Unable to sign message");
            return { data: t43.data.signature };
          }
          case "eth_signTransaction": {
            let [e40] = c5.params, t43 = await e30(i23, i23.signRequest, { chain_type: "ethereum", method: "eth_signTransaction", wallet_id: o18.id, params: { transaction: t34(e40) } });
            if ("eth_signTransaction" !== t43.method) throw Error("Unable to sign transaction");
            return { data: t43.data.signed_transaction };
          }
          case "eth_signTypedData_v4": {
            let [, e40] = c5.params, t43 = await e30(i23, i23.signRequest, { chain_type: "ethereum", method: c5.method, wallet_id: o18.id, params: { typed_data: e31(e40) } });
            if ("eth_signTypedData_v4" !== t43.method) throw Error("Unable to sign typed data");
            return { data: t43.data.signature };
          }
          case "eth_sign": {
            let [, n9] = c5.params, r20 = await e30(i23, i23.signRequest, { chain_type: "ethereum", method: "secp256k1_sign", wallet_id: o18.id, params: { hash: t33(n9) ? n9 : e28(f(n9)) } });
      ...
      })({ context: i22, account: o17, rpcRequest: c4.request });
    case "solana":
      return (async function({ context: e40, account: t43, rpcRequest: a20 }) {
        if ("signMessage" === a20.method) {
          let { message: n9 } = a20.params, r20 = await e30(e40, e40.signRequest, { chain_type: "solana", method: "signMessage", wallet_id: t43.id, params: { message: n9, encoding: "base64" } });
...
var p12 = class extends eventemitter3_default {
  async request(e40) {
    if (d3.has(e40.method)) return this.handleIFrameRpc(e40);
    switch (e40.method) {
      case "eth_accounts":
      ...
      case "wallet_switchEthereumChain":
        return this.handleSwitchEthereumChain(e40);
      case "eth_estimateGas":
        return this.handleEstimateGas(e40);
      case "eth_signTransaction": {
        let t43 = e40.params?.[0];
        return this.handleSignTransaction(t43);
      }
      case "eth_sendTransaction": {
        let t43 = e40.params?.[0];
        return this.handleSendTransaction(t43);
      }
      case "eth_populateTransactionRequest": {
        let t43 = e40.params?.[0];
        return this.handlePopulateTransaction(t43);
      }
      default:
        return this.handleJsonRpc(e40);
    }
  }
  ensureChainId(e40) {
    let t43 = { chainId: this._chainId, ...e40 };
    return this.internalSwitchEthereumChain(t43.chainId), t43;
  }
  internalSwitchEthereumChain(e40) {
    e40 && Number(e40) !== this._chainId && (this._chainId = Number(e40), this._client = p11(this._chainId, this._chains, { rpcUrls: [] }, { appId: this._privyInternal.appId }), this.emit("chainChanged", e40));
  }
  async handlePopulateTransaction(e40) {
    let t43 = this.ensureChainId(e40), i22 = t32(t43), { type: s11, ...n9 } = await this._client.prepareTransactionRequest({ account: t43.from ?? this._account.address, ...i22 });
    return { ...n9, type: i11[s11] };
  }
  async handleSignTransaction(e40) {
    let a20 = { ...e40 };
    for (let e41 of Object.keys(a20)) {
      let r20 = a20[e41];
      r20 && "bigint" == typeof r20 && (a20[e41] = toHex(r20));
    }
    return await this.handleIFrameRpc({ method: "eth_signTransaction", params: [a20] });
  }
  async handleSendTransaction(e40) {
    let t43 = await this.handlePopulateTransaction(e40), a20 = await this.handleSignTransaction(t43);
    return await this.handleJsonRpc({ method: "eth_sendRawTransaction", params: [a20] });
  }
  async handleEstimateGas(e40) {
    if (!e40.params || !Array.isArray(e40.params)) throw Error("Invalid params for eth_estimateGas");
    let t43 = e40.params?.[0], r20 = this.ensureChainId(t43), i22 = t32(r20);
    return await this._client.estimateGas({ account: r20.from ?? this._account.address, ...i22 });
  }
  handleSwitchEthereumChain(e40) {
    let t43;
    if (!e40.params || !Array.isArray(e40.params)) throw new d2(`Invalid params for ${e40.method}`, 4200);
    if ("string" == typeof e40.params[0]) t43 = e40.params[0];
    else {
      if (!("chainId" in e40.params[0]) || "string" != typeof e40.params[0].chainId) throw new d2(`Invalid params for ${e40.method}`, 4200);
      t43 = e40.params[0].chainId;
    }
    this.internalSwitchEthereumChain(t43);
  }
  async handleIFrameRpc(e40) {
    try {
      let t43 = await this._privyInternal.getAccessTokenInternal();
      if (!t43) throw Error("Missing privy token. User must be logged in");
      this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_started", { method: e40.method, address: this._account.address });
      let a20 = this._account;
      if (e25(a20)) {
        let { data: r20 } = await i13({ context: { app: this._appApi, fetchPrivyRoute: (...e41) => this._privyInternal.fetch(...e41), getCompiledPath: (...e41) => this._privyInternal.getPath(...e41), signRequest: ({ message: e41 }) => this._walletProxy.signWithUserSigner({ accessToken: t43, message: e41 }) }, account: a20, rpcRequest: { chainType: "ethereum", request: e40 } });
        return r20;
      }
      try {
        await this._walletProxy.connect({ entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, accessToken: t43 });
      } catch (e41) {
        let a21 = E(e41);
        if (a21 && "privy" === this._account.recovery_method) await this._walletProxy.recover({ entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, accessToken: t43 });
        else {
          if (!a21 || !this._onNeedsRecovery) throw e41;
          {
            let e42;
            await new Promise((async (t44, a22) => {
              e42 = setTimeout((() => a22(new e24({ code: "embedded_wallet_recovery_error", error: "User-owned recovery timed out" }))), 12e4), await this._onNeedsRecovery?.({ recoveryMethod: this._account.recovery_method, onRecovered: () => t44(true) });
            })).finally((() => clearTimeout(e42)));
          }
        }
      }
      return (await this._walletProxy.rpcWallet({ accessToken: t43, request: e40, entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, hdWalletIndex: this._account.wallet_index, chainType: "ethereum" })).response.data;
    } catch (t43) {
      console.error(t43);
      let a20 = t43 instanceof Error ? t43.message : "Unable to make wallet request";
      throw this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_failed", { method: e40.method, address: this._account.address, error: a20 }), new e24({ code: "embedded_wallet_request_error", error: a20 });
    }
  }
  async handleJsonRpc(e40) {
    return this._client.request(e40);
  }
  toJSON() {
    return `PrivyEIP1193Provider { address: '${this._account.address}', chainId: ${this._chainId}, request: [Function] }`;
  }
  constructor({ walletProxy: e40, privyInternal: t43, account: a20, entropyId: r20, entropyIdVerifier: i22, chains: s11, onNeedsRecovery: n9, chainId: o17 = s11[0].id, appApi: h10 }) {
    super(), this._walletProxy = e40, this._privyInternal = t43, this._account = a20, this._entropyId = r20, this._entropyIdVerifier = i22, this._chainId = o17, this._chains = s11, this._onNeedsRecovery = n9, this._client = p11(o17, s11, { rpcUrls: [] }, { appId: h10.appId }), this._appApi = h10;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/solana/isVersionedTransaction.mjs
function n5(n9) {
  return "version" in n9;
}

// node_modules/@privy-io/js-sdk-core/dist/esm/solana/getWalletPublicKeyFromTransaction.mjs
function e32(e40, t43) {
  let n9 = (n5(e40) ? e40.message : e40.compileMessage()).staticAccountKeys.find(((o17) => o17.toBase58() === t43));
  if (!n9) throw Error(`Transaction does not contain public key ${t43}`);
  return n9;
}
...
var i14 = class {
  async request(t43) {
    ...
    switch (t43.method) {
      case "signAndSendTransaction":
        return await this.handleSignAndSendTransaction(t43);
      case "signTransaction":
        return await this.handleSignTransaction(t43);
      default:
        return await this.handleIFrameRpc(t43);
    }
  }
  get _publicKey() {
    return this._account.address;
  }
  async connectAndRecover(t43) {
    if ("privy-v2" !== this._account.recovery_method) try {
      await this._proxy.connect({ entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, accessToken: t43 });
    } catch (r20) {
      let a20 = E(r20);
      if (a20 && "privy" === this._account.recovery_method) await this._proxy.recover({ entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier, accessToken: t43 });
      else {
        if (!a20 || !this._onNeedsRecovery) throw r20;
        {
          let t44;
          await new Promise((async (r21, a21) => {
            t44 = setTimeout((() => a21(new e24({ code: "embedded_wallet_recovery_error", error: "User-owned recovery timed out" }))), 12e4), await this._onNeedsRecovery?.({ recoveryMethod: this._account.recovery_method, onRecovered: () => r21(true) });
          })).finally((() => clearTimeout(t44)));
        }
      }
    }
  }
  async signMessageRpc(e40, t43) {
    let r20 = this._account;
    if (!e25(r20)) return (await this._proxy.rpcWallet({ accessToken: t43, request: e40, chainType: "solana", hdWalletIndex: this._account.wallet_index, entropyId: this._entropyId, entropyIdVerifier: this._entropyIdVerifier })).response.data;
    {
      let { data: a20 } = await i13({ context: { app: this._app, fetchPrivyRoute: (...e41) => this._privyInternal.fetch(...e41), getCompiledPath: (...e41) => this._privyInternal.getPath(...e41), signRequest: ({ message: e41 }) => this._proxy.signWithUserSigner({ accessToken: t43, message: e41 }) }, account: r20, rpcRequest: { chainType: "solana", request: e40 } });
  ...
  async handleIFrameRpc(t43) {
    try {
      let e40 = await this._privyInternal.getAccessTokenInternal();
      if (!e40) throw Error("Missing privy token. User must be logged in");
      return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_started", { method: t43.method, address: this._account.address }), await this.connectAndRecover(e40), await this.signMessageRpc(t43, e40);
    } catch (r20) {
      console.error(r20);
      let a20 = r20 instanceof Error ? r20.message : "Unable to make wallet request";
      throw this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_failed", { method: t43.method, address: this._account.address, error: a20 }), new e24({ code: "embedded_wallet_request_error", error: a20 });
    }
  }
  async handleSignAndSendTransaction(a20) {
    try {
      let e40 = await this._privyInternal.getAccessTokenInternal();
      if (!e40) throw Error("Missing privy token. User must be logged in");
      this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_started", { method: a20.method, address: this._account.address }), await this.connectAndRecover(e40);
      let { transaction: s11, connection: n9, options: i22 } = a20.params, o17 = e32(s11, this._account.address), c4 = n5(s11) ? Buffer.from(s11.message.serialize()) : s11.serializeMessage(), { signature: d7 } = await this.signMessageRpc({ method: "signMessage", params: { message: c4.toString("base64") } }, e40);
      return s11.addSignature(o17, Buffer.from(d7, "base64")), { signature: await n9.sendRawTransaction(s11.serialize(), i22) };
    } catch (t43) {
      console.error(t43);
      let r20 = t43 instanceof Error ? t43.message : "Unable to make wallet request";
  ...
  async handleSignTransaction(a20) {
    try {
      ...
      if (!e40) throw Error("Missing privy token. User must be logged in");
      this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_rpc_started", { method: a20.method, address: this._account.address }), await this.connectAndRecover(e40);
      let { transaction: s11 } = a20.params, n9 = e32(s11, this._account.address), i22 = n5(s11) ? Buffer.from(s11.message.serialize()) : s11.serializeMessage(), { signature: o17 } = await this.signMessageRpc({ method: "signMessage", params: { message: i22.toString("base64") } }, e40);
      return s11.addSignature(n9, Buffer.from(o17, "base64")), { signedTransaction: s11 };
    } catch (t43) {
      console.error(t43);
      let r20 = t43 instanceof Error ? t43.message : "Unable to make wallet request";
  ...
  constructor({ proxy: e40, privyInternal: t43, account: r20, entropyId: a20, entropyIdVerifier: s11, onNeedsRecovery: n9, app: i22 }) {
    this._proxy = e40, this._privyInternal = t43, this._account = r20, this._entropyId = a20, this._entropyIdVerifier = s11, this._onNeedsRecovery = n9, this._app = i22;
  }
};
...
var y = class {
  setMessagePoster(e40) {
    this._proxy = new h8(e40, this._mfaPromises), this._mfa.setProxy(this._proxy);
  }
  async signWithUserSigner(e40) {
    ...
    let { signature: o17 } = await this._proxy.signWithUserSigner({ accessToken: r20, message: e40.message });
    return { signature: o17 };
  }
  async add(e40) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    if ("user-controlled-server-wallets-only" === this._privyInternal.config?.embedded_wallet_config.mode) await t31({ context: { app: this._appApi, fetchPrivyRoute: (...e41) => this._privyInternal.fetch(...e41), getCompiledPath: (...e41) => this._privyInternal.getPath(...e41) }, chainType: e40.chainType });
    else {
      ...
      await this._proxy.addWallet({ accessToken: r21, ...e40 });
    }
    let { user: r20 } = await this._privyInternal.refreshSession();
    return { user: r20 };
  }
  async getBitcoinProvider({ wallet: e40, entropyId: r20, entropyIdVerifier: t43, recoveryPassword: i22, recoveryAccessToken: d7, recoverySecretOverride: a20 }) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    if (!await this._privyInternal.getAccessTokenInternal()) throw Error("User must be logged in to create an embedded wallet");
    return e25(e40) || await this._load({ entropyId: r20, entropyIdVerifier: t43, wallet: e40, recoveryPassword: i22, recoveryAccessToken: d7, recoverySecretOverride: a20 }), new r7({ account: e40, privyInternal: this._privyInternal, proxy: this._proxy, entropyId: r20, entropyIdVerifier: t43 });
  }
  async create({ password: e40, recoveryMethod: r20, recoveryToken: o17, recoveryKey: i22, recoverySecretOverride: a20, iCloudRecordNameOverride: s11, solanaAccount: n9, skipCallbacks: c4, idempotencyKey: l7 }) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    if ("user-controlled-server-wallets-only" === this._privyInternal.config?.embedded_wallet_config.mode) {
      ...
      await t31({ context: { app: this._appApi, fetchPrivyRoute: (...e41) => this._privyInternal.fetch(...e41), getCompiledPath: (...e41) => this._privyInternal.getPath(...e41) }, chainType: "ethereum", idempotencyKey: l7 });
    } else {
      let t43;
      if (t43 = r20 || (e40 ? "user-passcode" : "privy"), e40 && "string" != typeof e40) throw Error("Invalid recovery password, must be a string");
      if ("privy" === t43 && this._privyInternal.config?.embedded_wallet_config.require_user_password_on_create) throw Error("Password not provided yet is required by App configuration");
      ...
      let { address: c5 } = await this._proxy.create({ accessToken: d7, recoveryMethod: t43, recoveryKey: i22, recoveryPassword: e40, recoveryAccessToken: o17, recoverySecretOverride: a20, iCloudRecordNameOverride: s11, solanaAddress: n9?.address });
      if (!c5) throw Error("Failed to create wallet");
    }
    return await this._privyInternal.refreshSession(c4);
  }
  async createSolana(e40) {
    if (!this._proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_creation_error" });
    if ("user-controlled-server-wallets-only" === this._privyInternal.config?.embedded_wallet_config.mode) await t31({ context: { app: this._appApi, fetchPrivyRoute: (...e41) => this._privyInternal.fetch(...e41), getCompiledPath: (...e41) => this._privyInternal.getPath(...e41) }, chainType: "solana", idempotencyKey: e40?.idempotencyKey });
    else {
      let r20 = await this._privyInternal.getAccessTokenInternal();
      if (!r20) throw new e24({ error: "User must be logged in to create an embedded wallet", code: "embedded_wallet_creation_error" });
      e40?.ethereumAccount && await this.getProvider(e40.ethereumAccount);
      let { publicKey: o17 } = await this._proxy.createSolana({ accessToken: r20, ethereumAddress: e40?.ethereumAccount?.address });
      if (!o17) throw new e24({ error: "Failed to create wallet", code: "embedded_wallet_creation_error" });
    }
    return await this._privyInternal.refreshSession();
  }
  async delegateWallets({ delegatedWallets: e40, rootWallet: r20 }) {
    if (!this._proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_creation_error" });
    let o17 = await this._privyInternal.getAccessTokenInternal();
    if (!o17) throw new e24({ error: "User must be logged in to create an embedded wallet", code: "embedded_wallet_creation_error" });
    await this._proxy.delegateWallets({ accessToken: o17, delegatedWallets: e40, rootWallet: r20 });
  }
  async getProvider(e40, r20, t43, o17, i22) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    return e25(e40) || await this._load({ wallet: e40, entropyId: e40.address, entropyIdVerifier: "ethereum-address-verifier", recoveryPassword: r20, recoveryKey: i22, recoveryAccessToken: t43, recoverySecretOverride: o17 }), new p12({ account: e40, entropyId: e40.address, entropyIdVerifier: "ethereum-address-verifier", privyInternal: this._privyInternal, chains: this._chains, walletProxy: this._proxy, appApi: this._appApi });
  }
  async getEthereumProvider({ wallet: e40, entropyId: r20, entropyIdVerifier: t43, recoveryPassword: o17, recoveryAccessToken: i22, recoverySecretOverride: d7, recoveryKey: a20, onNeedsRecovery: n9 }) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    if (!await this._privyInternal.getAccessTokenInternal()) throw Error("User must be logged in to create an embedded wallet");
    return e25(e40) || (!n9 || o17 || i22 || d7 || a20) && await this._load({ entropyId: r20, entropyIdVerifier: t43, wallet: e40, recoveryPassword: o17, recoveryAccessToken: i22, recoverySecretOverride: d7, recoveryKey: a20 }), new p12({ account: e40, entropyId: r20, entropyIdVerifier: "ethereum-address-verifier", privyInternal: this._privyInternal, chains: this._chains, walletProxy: this._proxy, onNeedsRecovery: n9, appApi: this._appApi });
  }
  async getSolanaProvider(e40, r20, o17, i22, d7, a20, n9) {
    if (!this._proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_webview_not_loaded" });
    return e25(e40) || (!n9 || i22 || d7 || a20) && await this._load({ wallet: e40, entropyId: r20, entropyIdVerifier: o17, recoveryPassword: i22, recoveryAccessToken: d7, recoverySecretOverride: a20 }), new i14({ account: e40, privyInternal: this._privyInternal, proxy: this._proxy, entropyId: r20, entropyIdVerifier: o17, onNeedsRecovery: n9, app: this._appApi });
  }
  async setRecovery(e40) {
    let { wallet: r20, ...o17 } = e40;
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    if (e25(r20)) throw new e24({ error: "This wallet does not support setting recovery methods", code: "unsupported_recovery_method" });
    i10({ currentRecoveryMethod: r20.recovery_method, upgradeToRecoveryMethod: "icloud-native" === o17.recoveryMethod ? "icloud" : o17.recoveryMethod }), await this._load("solana" === r20.chain_type ? { wallet: r20, entropyId: r20.address, entropyIdVerifier: "solana-address-verifier" } : { wallet: r20, entropyId: r20.address, entropyIdVerifier: "ethereum-address-verifier" });
    let i22 = await this._privyInternal.getAccessTokenInternal();
    if (!i22) throw Error("User must be logged in to interact with embedded wallets");
    let d7 = r20.recovery_method;
    this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_set_recovery_started", { address: r20.address, target_recovery_method: o17.recoveryMethod, existing_recovery_method: d7 });
    try {
      let e41;
      if ("user-passcode" === o17.recoveryMethod) e41 = { recoveryMethod: "user-passcode", recoveryPassword: o17.password };
      else if ("google-drive" === o17.recoveryMethod) e41 = { recoveryMethod: "google-drive", recoveryAccessToken: o17.recoveryAccessToken };
      else if ("icloud" === o17.recoveryMethod) e41 = { recoveryMethod: "icloud", recoveryAccessToken: o17.recoveryAccessToken };
      else if ("icloud-native" === o17.recoveryMethod) e41 = { recoveryMethod: "icloud-native", iCloudRecordNameOverride: o17.iCloudRecordNameOverride, recoverySecretOverride: o17.recoverySecretOverride };
      else if ("recovery-encryption-key" === o17.recoveryMethod) e41 = { recoveryMethod: "recovery-encryption-key", recoveryKey: o17.recoveryKey };
      else {
        if ("privy" !== o17.recoveryMethod) throw Error(`Unknown recovery method: ${o17.recoveryMethod}`);
        e41 = { recoveryMethod: "privy" };
      }
      await this._proxy.setRecovery({ accessToken: i22, entropyId: r20.address, entropyIdVerifier: "solana" === r20.chain_type ? "solana-address-verifier" : "ethereum-address-verifier", ...e41 }), this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_set_recovery_completed", { address: r20.address, target_recovery_method: o17.recoveryMethod, existing_recovery_method: d7 });
      let { user: t43 } = await this._privyInternal.refreshSession();
      return { user: t43, provider: "ethereum" !== r20.chain_type ? null : new p12({ account: r20, entropyId: r20.address, entropyIdVerifier: "ethereum-address-verifier", privyInternal: this._privyInternal, chains: this._chains, walletProxy: this._proxy, appApi: this._appApi }) };
    } catch (e41) {
      throw this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_set_recovery_failed", { address: r20.address, recovery_method: r20.recovery_method, error: e41 instanceof Error ? e41.message : "Unable to recover wallet" }), e41;
    }
  }
  getURL() {
    let e40 = new URL(`${this._privyInternal.baseUrl}/apps/${this._privyInternal.appId}/embedded-wallets`);
    return this._privyInternal.caid && e40.searchParams.append("caid", this._privyInternal.caid), this._privyInternal.appClientId && e40.searchParams.append("client_id", this._privyInternal.appClientId), e40.href;
  }
  get chains() {
    return this._chains;
  }
  onMessage(e40) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    return this._proxy.handleEmbeddedWalletMessages(e40);
  }
  reload() {
  ...
  async ping(e40) {
    try {
      if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
      return await this._proxy.ping(e40), true;
    } catch (e41) {
      return console.error(e41), false;
    }
  }
  async _load({ entropyId: e40, entropyIdVerifier: r20, wallet: t43, recoveryPassword: o17, recoveryKey: d7, recoveryAccessToken: a20, recoverySecretOverride: s11 }) {
    if (!this._proxy) throw Error("Embedded wallet proxy not initialized");
    let n9 = await this._privyInternal.getAccessTokenInternal();
    if (!n9) throw Error("User must be logged in to interact with embedded wallets");
    try {
      return await this._proxy.connect({ accessToken: n9, entropyId: e40, entropyIdVerifier: r20 }), e40;
    } catch (c4) {
      if (E(c4)) try {
        if ("privy" === t43.recovery_method) {
          this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_started", { address: t43.address, recovery_method: t43.recovery_method });
          let o18 = await this._proxy.recover({ accessToken: n9, entropyId: e40, entropyIdVerifier: r20 });
          return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_completed", { address: t43.address, recovery_method: t43.recovery_method }), o18.entropyId;
        }
        if ("user-passcode" === t43.recovery_method && o17) {
          this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_started", { address: t43.address, recovery_method: t43.recovery_method });
          let i22 = await this._proxy.recover({ accessToken: n9, recoveryPassword: o17, entropyId: e40, entropyIdVerifier: r20 });
          return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_completed", { address: t43.address, recovery_method: t43.recovery_method }), i22.entropyId;
        }
        if (["google-drive", "icloud"].includes(t43.recovery_method) && a20) {
          this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_started", { address: t43.address, recovery_method: t43.recovery_method });
          let o18 = await this._proxy.recover({ accessToken: n9, recoveryAccessToken: a20, entropyId: e40, entropyIdVerifier: r20 });
          return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_completed", { address: t43.address, recovery_method: t43.recovery_method }), o18.entropyId;
        }
        if ("icloud" === t43.recovery_method && s11) {
          this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_started", { address: t43.address, recovery_method: "icloud-native" });
          let o18 = await this._proxy.recover({ accessToken: n9, recoverySecretOverride: s11, entropyId: e40, entropyIdVerifier: r20 });
          return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_completed", { address: t43.address, recovery_method: "icloud-native" }), o18.entropyId;
        }
        if ("recovery-encryption-key" === t43.recovery_method && d7) {
          this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_started", { address: t43.address, recovery_method: t43.recovery_method });
          let o18 = await this._proxy.recover({ accessToken: n9, recoveryKey: d7, entropyId: e40, entropyIdVerifier: r20 });
          return this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_completed", { address: t43.address, recovery_method: t43.recovery_method }), o18.entropyId;
        }
      } catch (r21) {
        throw this._privyInternal.createAnalyticsEvent("embedded_wallet_sdk_recovery_failed", { address: t43.address, recovery_method: t43.recovery_method, error: r21 instanceof Error ? r21.message : `Unable to recover wallet: ${e40}` }), r21;
      }
      throw c4;
    }
  }
  constructor(t43, o17, i22, d7, a20, s11) {
    if (this._chains = Array.from(H), this._privyInternal = t43, o17 && (this._proxy = new h8(o17, a20), d7.setProxy(this._proxy)), i22) {
      let e40 = t27(i22);
      this._chains = e40;
    }
...
var byteToHex = [];
for (let i22 = 0; i22 < 256; ++i22) {
  byteToHex.push((i22 + 256).toString(16).slice(1));
}
...
function init(converter, defaultAttributes) {
  ...
  function get(name) {
    ...
    var jar = {};
    for (var i22 = 0; i22 < cookies.length; i22++) {
      var parts = cookies[i22].split("=");
...
var e33 = Promise.allSettled.bind(Promise) ?? ((e40) => Promise.all(e40.map(((e41) => e41.then(((e42) => ({ status: "fulfilled", value: e42 }))).catch(((e42) => ({ status: "rejected", reason: e42 })))))));
...
var k = class extends eventemitter3_default {
  set isUsingServerCookies(e40) {
    this._isUsingServerCookies = e40;
  }
  async getCustomerAccessToken() {
    let e40 = await this._storage.get(o11);
    try {
      return "string" == typeof e40 ? new t13(e40).value : null;
    } catch (e41) {
      return console.error(e41), await this.destroyLocalState({ reason: "getToken_error" }), null;
    }
  }
  async getPrivyAccessToken() {
    let e40 = await this._storage.get(a17);
    try {
      return "string" == typeof e40 ? new t13(e40).value : null;
    } catch (e41) {
      return console.error(e41), await this.destroyLocalState({ reason: "getToken_error" }), null;
  ...
  async getRefreshToken() {
    let e40 = await this._storage.get(_3);
    return "string" == typeof e40 ? e40 : null;
  }
  async getIdentityToken() {
    let e40 = await this._storage.get(g);
    return "string" == typeof e40 ? e40 : null;
  }
  get mightHaveServerCookies() {
    try {
      let e40 = api.get(c);
      return void 0 !== e40 && e40.length > 0;
    } catch (e40) {
      console.error(e40);
    }
    return false;
  }
  hasRefreshCredentials(e40, t43) {
    return this.mightHaveServerCookies || "string" == typeof e40 && "string" == typeof t43;
  }
  tokenIsActive(e40) {
    if (!e40) return false;
    let t43 = t13.parse(e40);
    return null !== t43 && !t43.isExpired(30);
  }
  async destroyLocalState(e40) {
    await e33([this._storage.del(o11), this._storage.del(a17), this._storage.del(_3), this._storage.del(g), this._storage.del(this.GUEST_CREDENTIAL_STORAGE_KEY)]), api.remove(n6), api.remove(h9), api.remove(l3), api.remove(c), e40?.reason && this.emit("storage_cleared", { reason: e40.reason });
  }
  async storeCustomerAccessToken(e40) {
    if ("string" == typeof e40) {
      let t43 = await this._storage.get(o11);
      if (await this._storage.put(o11, e40), !this._isUsingServerCookies) {
        let t44 = t13.parse(e40)?.expiration;
        api.set(n6, e40, { sameSite: "Strict", secure: true, expires: t44 ? new Date(1e3 * t44) : void 0 });
      }
      t43 !== e40 && this.emit("token_stored", { cookiesEnabled: this._isUsingServerCookies });
    } else {
      let e41 = await this._storage.get(o11);
      await this._storage.del(o11), api.remove(n6), null !== e41 && this.emit("token_cleared", { reason: "set_with_non_string_value" });
    }
  }
  async storePrivyAccessToken(e40) {
    "string" == typeof e40 ? await this._storage.put(a17, e40) : await this._storage.del(a17);
  }
  async storeRefreshToken(e40) {
    "string" == typeof e40 ? (await this._storage.put(_3, e40), this._isUsingServerCookies || (api.set(c, "t", { sameSite: "Strict", secure: true, expires: 30 }), api.set(h9, e40, { sameSite: "Strict", secure: true, expires: 30 })), this.emit("refresh_token_stored", { cookiesEnabled: this._isUsingServerCookies })) : (await this._storage.del(_3), api.remove(h9), api.remove(c), this.emit("refresh_token_cleared", { reason: "set_with_non_string_value" }));
  }
  async updateWithTokensResponse(e40) {
    let t43 = (await e33([this.storeCustomerAccessToken(e40.token), this.storePrivyAccessToken(e40.privy_access_token), this.storeRefreshToken(e40.refresh_token), this.storeIdentityToken(e40.identity_token), this.processOAuthTokens(e40.oauth_tokens)])).filter(((e41) => "rejected" === e41.status));
    t43.length > 0 && this.emit("error_storing_tokens", t43.map(((e41) => String(e41.reason))).join(", "));
  }
  async processOAuthTokens(e40) {
    e40 && this.emit("oauth_tokens_granted", e40);
  }
  async storeIdentityToken(e40) {
    if ("string" == typeof e40) {
      let t43 = await this._storage.get(g);
      if (await this._storage.put(g, e40), !this._isUsingServerCookies) {
        let t44 = t13.parse(e40)?.expiration;
        api.set(l3, e40, { sameSite: "Strict", secure: true, expires: t44 ? new Date(1e3 * t44) : void 0 });
      }
      t43 !== e40 && this.emit("identity_token_stored", { cookiesEnabled: this._isUsingServerCookies });
    } else {
      let e41 = await this._storage.get(g);
      await this._storage.del(g), api.remove(l3), null !== e41 && this.emit("identity_token_cleared", { reason: "set_with_non_string_value" });
    }
  }
  async getOrCreateGuestCredential() {
    let e40 = await this._storage.get(this.GUEST_CREDENTIAL_STORAGE_KEY);
    if (e40 && "string" == typeof e40) return e40;
  ...
  constructor(e40) {
    super(), this._isUsingServerCookies = false, this._storage = e40.storage, this.GUEST_CREDENTIAL_STORAGE_KEY = `privy:guest:${e40.appId}`;
  }
};
...
var t35 = (t43) => {
  let e40 = new AbortController();
  return setTimeout((() => e40.abort()), t43), e40.signal;
};

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/toSearchParams.mjs
function r15(r20) {
  let n9 = new URLSearchParams();
  for (let t43 in r20) null != r20[t43] && n9.append(t43, String(r20[t43]));
  return Array.from(n9).length ? "?" + n9.toString() : "";
}
...
var e34 = { NONE: Number.NEGATIVE_INFINITY, ERROR: 1, WARN: 2, INFO: 3, DEBUG: Number.POSITIVE_INFINITY };
var N = ({ level: N2, logger: o17 }) => ({ get level() {
  return N2;
}, error: e34[N2] >= e34.ERROR ? o17.error : o12, warn: e34[N2] >= e34.WARN ? o17.warn : o12, info: e34[N2] >= e34.INFO ? o17.info : o12, debug: e34[N2] >= e34.DEBUG ? o17.debug : o12 });

// node_modules/@privy-io/js-sdk-core/dist/esm/client/PrivyInternal.mjs
var p13 = "privy:caid";
var u = class {
  setCallbacks(e40) {
    this.callbacks = { ...this.callbacks, ...e40 };
  }
  get isReady() {
    return !!this._config;
  }
  ...
  getPath(e40, { params: t43, query: s11 }) {
    return `${this.baseUrl}${e6(e40.path, t43)}${r15(s11)}`;
  }
  async fetch(e40, { body: t43, params: s11, query: i22, headers: r20, onRequest: a20 = this._beforeRequest.bind(this) }) {
    let o17 = new Request(this.getPath(e40, { params: s11, query: i22 }), { method: e40.method, body: JSON.stringify(t43), headers: r20 }), n9 = await a20(o17), c4 = await this._fetch(o17, n9), l7 = await c4.json();
    if (c4.status > 299) throw new r6({ ...l7, status: c4.status });
    return l7;
  }
  async _beforeRequestWithoutInitialize(e40) {
    let t43 = await this.session.getPrivyAccessToken() ?? await this.session.getCustomerAccessToken(), s11 = new Headers(e40.headers);
    s11.set("privy-app-id", this.appId), this.appClientId && s11.set("privy-client-id", this.appClientId), s11.set("privy-client", this._sdkVersion), t43 && s11.set("Authorization", `Bearer ${t43}`), s11.set("Content-Type", "application/json"), s11.set("Accept", "application/json");
    let i22 = await this._getOrGenerateClientAnalyticsId();
    return i22 && s11.set("privy-ca-id", i22), this.nativeAppIdentifier && s11.set("x-native-app-identifier", this.nativeAppIdentifier), { signal: t35(2e4), headers: s11, credentials: "include" };
  }
  async beforeRequestWithoutRefresh(e40) {
    return await this._initialize(), this._beforeRequestWithoutInitialize(e40);
  }
  async _beforeRequest(e40) {
    return await this._initialize(), await this.getAccessTokenInternal(), this.beforeRequestWithoutRefresh(e40);
  }
  async getAppConfig() {
  ...
  async _getOrGenerateClientAnalyticsId() {
    ...
    try {
      let e40 = await this._storage.get(p13);
      if ("string" == typeof e40 && e40.length > 0) return this._analyticsId = e40, e40;
    } catch (e40) {
      this.logger.error("Unable to load clientId", e40);
    }
    try {
      this._analyticsId = v4_default();
    } catch (e40) {
      this.logger.error("Unable to generate uuidv4", e40);
    }
    if (this._analyticsId) {
      try {
        await this._storage.put(p13, this._analyticsId);
      } catch (e40) {
        this.logger.error(`Unable to store clientId: ${this._analyticsId}`, e40);
      }
      return this._analyticsId;
    }
  }
  async destroyClientAnalyticsId() {
    try {
      ...
      this.logger.error("Unable to delete clientId", e40);
    }
  }
  async createAnalyticsEvent(e40, t43) {
    try {
      await this.fetch(t, { body: { event_name: e40, client_id: await this._getOrGenerateClientAnalyticsId(), payload: t43 }, onRequest: this.beforeRequestWithoutRefresh.bind(this) });
    } catch (e41) {
    }
  }
  async refreshSession(e40 = false) {
    if (!await this.isStorageAccessible()) throw new e24({ code: "storage_error", error: "Unable to access storage" });
    let t43 = await this.session.getRefreshToken() ?? void 0, s11 = t43 ?? "key", i22 = this._cache.get(s11);
    if (i22) return this.logger.debug("[privy:refresh] found in-flight session refresh request, deduping"), await i22;
    let r20 = this._refreshSession(t43, e40);
  ...
  async _refreshSession(e40, t43) {
    let i22 = await this.session.getCustomerAccessToken();
    if (!this.session.hasRefreshCredentials(i22, e40 ?? null)) throw this.logger.debug("[privy:refresh] missing tokens, skipping request"), await this._initialize(), new r6({ code: I.MISSING_OR_INVALID_TOKEN, error: "No tokens found in storage", status: 401 });
    try {
      this.logger.debug(`[privy:refresh] fetching: ${s4.path}`);
      let s11 = await this.fetch(s4, { body: { refresh_token: e40 }, onRequest: this.beforeRequestWithoutRefresh.bind(this) }), i23 = s11.session_update_action;
      return this.logger.debug(`[privy:refresh] response: ${i23}`), "set" === i23 && (await this.session.updateWithTokensResponse(s11), this.logger.debug("[privy:refresh] tokens stored")), "clear" === i23 && (await this.session.destroyLocalState(), this.logger.debug("[privy:refresh] tokens cleared"), t43 || this.callbacks?.setUser?.(null)), "ignore" === i23 && (s11.token && (await this.session.storeCustomerAccessToken(s11.token), this.logger.debug("[privy:refresh] token stored (CAT)")), s11.privy_access_token && (await this.session.storePrivyAccessToken(s11.privy_access_token), this.logger.debug("[privy:refresh] token stored (PAT)")), s11.identity_token && (this.logger.debug("[privy:refresh] token stored (IDT)"), await this.session.storeIdentityToken(s11.identity_token))), this.logger.debug("[privy:refresh] returning response"), t43 || this.callbacks?.setUser?.(s11.user), s11;
    } catch (e41) {
      throw this.logger.debug(`[privy:refresh] error: ${e41.message ?? "unknown error"}`), e41 instanceof r6 && e41.code === I.MISSING_OR_INVALID_TOKEN && (await this.session.destroyLocalState(), t43 || this.callbacks?.setUser?.(null)), e41;
    }
  }
  async getAccessToken() {
    let [e40, t43] = await Promise.all([this.session.getCustomerAccessToken(), this.session.getRefreshToken()]);
    if (e40 && !this.session.tokenIsActive(e40) && this.session.hasRefreshCredentials(e40, t43)) {
      let t44 = await this.refreshSession(), s11 = await this.session.getCustomerAccessToken();
      return t44.token || this.logger.debug("[privy:getAccessToken] expected token received null"), t44.token === e40 && this.logger.debug("[privy:getAccessToken] expected new token in response received existing"), s11 === e40 && this.logger.debug("[privy:getAccessToken] expected new token in storage received existing"), t44.token ?? s11;
    }
    return e40 && !this.session.tokenIsActive(e40) ? (this.logger.debug("[privy:getAccessToken] unable to refresh inactive token"), this.callbacks?.setUser?.(null), await this.session.destroyLocalState(), null) : e40;
  }
  async getAccessTokenInternal() {
    let e40 = await this.getAccessToken();
    return await this.session.getPrivyAccessToken() ?? e40;
  }
  async getIdentityToken() {
  ...
  async isStorageAccessible() {
    try {
      let e40 = `privy:__storage__test-${v4_default()}`, s11 = "blobby";
      await this._storage.put(e40, s11);
      let i22 = await this._storage.get(e40);
      return await this._storage.del(e40), i22 === s11;
    } catch (e40) {
      return this.logger.error(e40), false;
    }
  }
  constructor(t43) {
    this._sdkVersion = "js-sdk-core:0.60.9", this._cache = /* @__PURE__ */ new Map(), this.logger = N({ level: t43.logger?.level ?? t43.logLevel ?? "ERROR", logger: t43.logger ?? console }), this._storage = t43.storage, this._analyticsId = null, this._getOrGenerateClientAnalyticsId(), this.baseUrl = t43.baseUrl ?? "https://auth.privy.io", this.appId = t43.appId, this.appClientId = t43.appClientId, this._sdkVersion = t43.sdkVersion ?? this._sdkVersion, this.callbacks = t43.callbacks, this.nativeAppIdentifier = t43.nativeAppIdentifier, this.session = new k({ storage: this._storage, isUsingServerCookies: false, appId: t43.appId }), this._fetch = (0, import_fetch_retry.default)(globalThis.fetch, { retries: 3, retryDelay: (e40) => 3 ** e40 * 500, retryOn: [408, 409, 425, 500, 502, 503, 504] }), this.session.on("error_storing_tokens", ((e40) => {
      this.createAnalyticsEvent("error_updating_tokens_in_storage", { reason: e40 });
    }));
  }
};
...
var e36 = (e40) => "wallet" === e40.type && "privy" === e40.wallet_client_type && "embedded" === e40.connector_type;

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/getAllUserEmbeddedEthereumWallets.mjs
var t37 = (t43) => t43 ? t43.linked_accounts.filter(e36).filter(((e40) => "ethereum" === e40.chain_type)).sort(((e40, t44) => e40.wallet_index - t44.wallet_index)) : [];

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/getUserEmbeddedEthereumWallet.mjs
var l4 = (l7) => t37(l7).find(((e40) => 0 === e40.wallet_index)) ?? null;

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/getAllUserEmbeddedSolanaWallets.mjs
var t38 = (t43) => t43 ? t43.linked_accounts.filter(e36).filter(((e40) => "solana" === e40.chain_type)).sort(((e40, t44) => e40.wallet_index - t44.wallet_index)) : [];

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/getUserEmbeddedSolanaWallet.mjs
var l5 = (l7) => t38(l7).find(((e40) => 0 === e40.wallet_index)) ?? null;

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/shouldCreateEmbeddedEthWallet.mjs
var t39 = (t43, l7) => !("off" === l7 || t37(t43).length > 0 || t43.linked_accounts.filter(((e40) => "wallet" === e40.type && "ethereum" === e40.chain_type)).length > 0 && "all-users" !== l7);

// node_modules/@privy-io/js-sdk-core/dist/esm/utils/shouldCreateEmbeddedSolWallet.mjs
var l6 = (l7, t43) => !("off" === t43 || t38(l7).length > 0 || l7.linked_accounts.filter(((e40) => "wallet" === e40.type && "solana" === e40.chain_type)).length > 0 && "all-users" !== t43);

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/maybeCreateWalletOnLogin.mjs
var o13 = async (o17, a20, l7) => {
  let u4 = t39(a20.user, l7?.ethereum?.createOnLogin ?? "off"), i22 = l6(a20.user, l7?.solana?.createOnLogin ?? "off");
  if (u4 && i22) {
    let t43 = await o17.create({ recoveryMethod: "privy", skipCallbacks: true });
    return { ...await o17.createSolana({ ethereumAccount: l4(t43.user) ?? void 0 }), is_new_user: a20.is_new_user, oauth_tokens: a20.oauth_tokens };
  }
  return u4 ? { ...await o17.create({ recoveryMethod: "privy", solanaAccount: l5(a20.user) ?? void 0 }), is_new_user: a20.is_new_user, oauth_tokens: a20.oauth_tokens } : i22 ? { ...await o17.createSolana({ ethereumAccount: l4(a20.user) ?? void 0 }), is_new_user: a20.is_new_user, oauth_tokens: a20.oauth_tokens } : a20;
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/CustomProviderApi.mjs
var i15 = class {
  async syncWithToken(t43, i22, r20) {
    let l7 = await this._privyInternal.fetch(t2, { body: { token: t43, mode: r20 } });
    await this._privyInternal.session.updateWithTokensResponse(l7);
    let a20 = await o13(this._embedded, l7, i22?.embedded);
    return this._privyInternal.callbacks?.setUser?.(a20.user), a20;
  }
  async linkWithToken(e40) {
    await this._privyInternal.fetch(a2, { body: { token: e40 } });
    let s11 = await this._privyInternal.refreshSession();
    return { user: s11.user, identity_token: s11.identity_token };
  }
  constructor(e40, t43) {
    this._privyInternal = e40, this._embedded = t43;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/EmailApi.mjs
var a18 = class {
  async sendCode(t43, s11) {
    return this._privyInternal.fetch(a7, { body: { email: t43, token: s11 } });
  }
  async loginWithCode(e40, s11, i22, r20) {
    let a20 = await this._privyInternal.fetch(s2, { body: { email: e40, code: s11, mode: i22 } });
    await this._privyInternal.session.updateWithTokensResponse(a20);
    let l7 = await o13(this._embedded, a20, r20?.embedded);
    return this._privyInternal.callbacks?.setUser?.(l7.user), l7;
  }
  async linkWithCode(e40, t43) {
    await this._privyInternal.fetch(p6, { body: { email: e40, code: t43 } });
    let i22 = await this._privyInternal.refreshSession();
    return { user: i22.user, identity_token: i22.identity_token };
  }
  async updateEmail({ oldEmailAddress: e40, newEmailAddress: t43, code: s11 }) {
    await this._privyInternal.fetch(e3, { body: { oldAddress: e40, newAddress: t43, code: s11 } });
    let r20 = await this._privyInternal.refreshSession();
    return { user: r20.user, identity_token: r20.identity_token };
  }
  async unlink(e40) {
    await this._privyInternal.fetch(t7, { body: { address: e40 } });
    let t43 = await this._privyInternal.refreshSession();
    return { user: t43.user, identity_token: t43.identity_token };
  }
  constructor(e40, t43) {
    this._privyInternal = e40, this._embedded = t43;
  }
...
var n7 = class {
  async initializeAuth({ relyingParty: t43, redirectUrl: r20, token: i22 }) {
    return await this._privyInternal.fetch(a3, { body: { relying_party: t43, redirect_url: r20, token: i22 } });
  }
  async getFarcasterStatus({ channel_token: e40 }) {
    return await this._privyInternal.fetch(i, { headers: { "farcaster-channel-token": e40 } });
  }
  async authenticate({ channel_token: e40, message: t43, signature: i22, fid: s11, mode: n9 }, l7) {
    let d7 = await this._privyInternal.fetch(t3, { body: { channel_token: e40, message: t43, signature: i22, fid: s11, mode: n9 } });
    await this._privyInternal.session.updateWithTokensResponse(d7);
    let o17 = await o13(this._embedded, d7, l7?.embedded);
    return this._privyInternal.callbacks?.setUser?.(o17.user), o17;
  }
  async link({ channel_token: e40, message: t43, signature: r20, fid: s11 }) {
    await this._privyInternal.fetch(e, { body: { channel_token: e40, message: t43, signature: r20, fid: s11 } });
    let a20 = await this._privyInternal.refreshSession();
    return { user: a20.user, identity_token: a20.identity_token };
  }
  async unlink({ fid: e40 }) {
    await this._privyInternal.fetch(h, { body: { fid: e40 } });
    let t43 = await this._privyInternal.refreshSession();
    return { user: t43.user, identity_token: t43.identity_token };
  }
  constructor(e40, t43) {
...
var i16 = class {
  ...
  async authenticate({ message: e40, signature: i22, fid: a20 }, l7) {
    let r20 = await this._privyInternal.fetch(r, { body: { message: e40, signature: i22, fid: a20 } });
    await this._privyInternal.session.updateWithTokensResponse(r20);
    let d7 = await o13(this._embedded, r20, l7?.embedded);
...
var s9 = class {
  async create(s11) {
    ...
    await this._privyInternal.session.updateWithTokensResponse(r20);
    let a20 = await o13(this._embedded, r20, s11?.embedded);
    return this._privyInternal.callbacks?.setUser?.(a20.user), a20;
  }
  constructor(e40, t43) {
    this._privyInternal = e40, this._embedded = t43;
...
var n8 = "privy:code_verifier";
async function r16(e40, t43) {
  let n9 = new TextEncoder().encode(e40);
  return new Uint8Array(await t43("SHA-256", n9));
}
function o14(e40) {
  return crypto.getRandomValues(new Uint8Array(e40));
}
function i17() {
  return base64url_exports.encode(o14(36));
}
function c2() {
  return i17();
}
async function u2({ codeVerifier: t43, method: n9 = "S256", digest: o17 = crypto.subtle.digest.bind(crypto.subtle) }) {
  if ("S256" != n9) return t43;
  {
    let n10 = await r16(t43, o17);
    return base64url_exports.encode(n10);
  }
}

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/OAuthApi.mjs
var m2 = class {
  async generateURL(t43, i22) {
    let s11 = i17(), r20 = c2(), h10 = await u2({ codeVerifier: s11, digest: this._crypto?.digest });
    return await Promise.all([this._storage.put(n8, s11), this._storage.put(t40, r20)]), this._privyInternal.fetch(a5, { body: { redirect_to: i22, provider: t43, code_challenge: h10, state_code: r20 } });
  }
  async loginWithCode(e40, i22, s11, a20, o17, l7) {
    let [m3, p14] = await Promise.all([this._storage.get(n8), this._storage.get(t40)]);
    if (p14 !== i22) throw this._privyInternal.createAnalyticsEvent("possible_phishing_attempt", { flow: "oauth", provider: s11, storedStateCode: p14 ?? "", returnedStateCode: i22 ?? "" }), new e24({ code: "pkce_state_code_mismatch", error: "Unexpected auth flow. This may be a phishing attempt." });
    let _4 = await this._privyInternal.fetch(t5, { body: { authorization_code: e40, code_type: a20, state_code: p14, code_verifier: m3, mode: o17 } });
    await this._privyInternal.session.updateWithTokensResponse(_4);
    let c4 = await o13(this._embedded, _4, l7?.embedded);
    return await Promise.all([this._storage.del(n8), this._storage.del(t40)]), this._privyInternal.callbacks?.setUser?.(c4.user), c4;
  }
  async linkWithCode(e40, t43, s11, a20) {
    let [o17, l7] = await Promise.all([this._storage.get(n8), this._storage.get(t40)]);
    if (l7 !== t43) throw this._privyInternal.createAnalyticsEvent("possible_phishing_attempt", { flow: "oauth", provider: s11, storedStateCode: l7 ?? "", returnedStateCode: t43 ?? "" }), new e24({ code: "pkce_state_code_mismatch", error: "Unexpected auth flow. This may be a phishing attempt." });
    let h10 = await this._privyInternal.fetch(h2, { body: { authorization_code: e40, code_type: a20, state_code: l7, code_verifier: o17 } });
    await this._privyInternal.session.processOAuthTokens(h10.oauth_tokens);
    let m3 = await this._privyInternal.refreshSession();
    return await Promise.all([this._storage.del(n8), this._storage.del(t40)]), { user: m3.user, identity_token: m3.identity_token };
  }
  async unlink(e40, t43) {
    await this._privyInternal.fetch(o, { body: { provider: e40, subject: t43 } });
    let i22 = await this._privyInternal.refreshSession();
    return { user: i22.user, identity_token: i22.identity_token };
  }
  constructor(e40, t43, i22, s11) {
    this._privyInternal = e40, this._embedded = t43, this._storage = i22, this._crypto = s11;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/PasskeyApi.mjs
var o15 = class {
  async generateRegistrationOptions(t43) {
    return await this._privyInternal.fetch(i2, { body: { relying_party: t43 } });
  }
  async generateAuthenticationOptions(e40) {
    return await this._privyInternal.fetch(e2, { body: { relying_party: e40 } });
  }
  async generateSignupOptions(e40) {
    return await this._privyInternal.fetch(p5, { body: { relying_party: e40 } });
  }
  async loginWithPasskey(e40, t43, s11, n9) {
    let i22 = await this._privyInternal.fetch(a6, { body: { relying_party: s11, challenge: t43, authenticator_response: this._transformAuthenticationResponseToSnakeCase(e40) } });
    await this._privyInternal.session.updateWithTokensResponse(i22);
    let o17 = await o13(this._embedded, i22, n9?.embedded);
    return this._privyInternal.callbacks?.setUser?.(o17.user), o17;
  }
  async signupWithPasskey(e40, t43, s11) {
    let a20 = await this._privyInternal.fetch(s, { body: { relying_party: t43, authenticator_response: this._transformRegistrationResponseToSnakeCase(e40) } });
    await this._privyInternal.session.updateWithTokensResponse(a20);
    let i22 = await o13(this._embedded, a20, s11?.embedded);
    return this._privyInternal.callbacks?.setUser?.(i22.user), i22;
  }
  async linkWithPasskey(e40, t43) {
    await this._privyInternal.fetch(t6, { body: { relying_party: t43, authenticator_response: this._transformRegistrationResponseToSnakeCase(e40) } });
    let s11 = await this._privyInternal.refreshSession();
    return { user: s11.user, identity_token: s11.identity_token };
  }
  _transformRegistrationResponseToSnakeCase(e40) {
    return { type: e40.type, id: e40.id, raw_id: e40.rawId, response: { client_data_json: e40.response.clientDataJSON, attestation_object: e40.response.attestationObject, authenticator_data: e40.response.authenticatorData || void 0, transports: e40.response.transports || void 0, public_key: e40.response.publicKey || void 0, public_key_algorithm: e40.response.publicKeyAlgorithm || void 0 }, authenticator_attachment: e40.authenticatorAttachment || void 0, client_extension_results: { app_id: e40.clientExtensionResults.appid || void 0, cred_props: e40.clientExtensionResults.credProps || void 0, hmac_create_secret: e40.clientExtensionResults.hmacCreateSecret || void 0 } };
  }
  _transformAuthenticationResponseToSnakeCase(e40) {
    return { type: e40.type, id: e40.id, raw_id: e40.rawId, response: { signature: e40.response.signature, client_data_json: e40.response.clientDataJSON, authenticator_data: e40.response.authenticatorData, user_handle: e40.response.userHandle || void 0 }, authenticator_attachment: e40.authenticatorAttachment || void 0, client_extension_results: { app_id: e40.clientExtensionResults.appid || void 0, cred_props: e40.clientExtensionResults.credProps || void 0, hmac_create_secret: e40.clientExtensionResults.hmacCreateSecret || void 0 } };
  }
  constructor(e40, t43) {
    this._privyInternal = e40, this._embedded = t43;
  }
...
var o16 = class {
  async sendCode(t43, r20) {
    return this._privyInternal.fetch(a8, { body: { phoneNumber: t43, token: r20 } });
  }
  async loginWithCode(e40, r20, i22, n9) {
    let o17 = await this._privyInternal.fetch(s3, { body: { phoneNumber: e40, code: r20, mode: i22 } });
    await this._privyInternal.session.updateWithTokensResponse(o17);
    let a20 = await o13(this._embedded, o17, n9?.embedded);
    return this._privyInternal.callbacks?.setUser?.(a20.user), a20;
  }
  async linkWithCode(e40, t43) {
    await this._privyInternal.fetch(p7, { body: { phoneNumber: e40, code: t43 } });
    let i22 = await this._privyInternal.refreshSession();
    return { user: i22.user, identity_token: i22.identity_token };
  }
  async updatePhone({ oldPhoneNumber: e40, newPhoneNumber: t43, code: r20 }) {
    await this._privyInternal.fetch(e4, { body: { old_phone_number: e40, new_phone_number: t43, code: r20 } });
    let n9 = await this._privyInternal.refreshSession();
    return { user: n9.user, identity_token: n9.identity_token };
  }
  async unlink(e40) {
    await this._privyInternal.fetch(t8, { body: { phoneNumber: e40 } });
    let t43 = await this._privyInternal.refreshSession();
    return { user: t43.user, identity_token: t43.identity_token };
  }
  constructor(e40, t43) {
...
var r17 = class {
  async unlinkWallet(t43) {
    await this._privyInternal.fetch(h4, { body: { address: t43 } });
    let i22 = await this._privyInternal.refreshSession();
    return { user: i22.user, identity_token: i22.identity_token };
  }
  async linkWithSiwe(e40, i22, n9) {
    let s11 = i22 || this._wallet, r20 = n9 || this._preparedMessage;
    ...
    await this._privyInternal.fetch(a10, { body: { message: r20, signature: e40, chainId: s11.chainId, walletClientType: s11.walletClientType ?? null, connectorType: s11.connectorType ?? null } });
    let a20 = await this._privyInternal.refreshSession();
    return { user: a20.user, identity_token: a20.identity_token };
  }
  async loginWithSiwe(e40, t43, n9, r20, a20) {
    let l7 = t43 || this._wallet, o17 = n9 || this._preparedMessage;
    if (!l7) throw Error("A wallet must be provided in the init step or as an argument to loginWithSiwe");
    if (!o17) throw Error("A message must be generated and signed before being used to login to privy with a wallet");
    let d7 = await this._privyInternal.fetch(i3, { body: { signature: e40, message: o17, chainId: l7.chainId, walletClientType: l7.walletClientType ?? null, connectorType: l7.connectorType ?? null, mode: r20 } });
    await this._privyInternal.session.updateWithTokensResponse(d7);
    let h10 = await o13(this._embedded, d7, a20?.embedded);
    return this._privyInternal.callbacks?.setUser?.(h10.user), h10;
  }
  async init(e40, t43, i22) {
    var s11;
    this._wallet = e40;
    let { nonce: r20 } = await this._privyInternal.fetch(t10, { body: { address: e40.address } }), a20 = `${(s11 = { chainId: e40.chainId.toString().replace("eip155:", ""), address: e40.address, issuedAt: (/* @__PURE__ */ new Date()).toISOString(), statement: "By signing, you are proving you own this wallet and logging in. This does not initiate a transaction or cost any fees.", domain: t43, nonce: r20, uri: i22 }).domain} wants you to sign in with your Ethereum account:
${s11.address}
...
- https://privy.io`;
  ...
  constructor(e40, t43) {
    this._wallet = void 0, this._privyInternal = e40, this._embedded = t43;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/auth/SiwsApi.mjs
var r18 = class {
  async unlink({ address: t43 }) {
    await this._privyInternal.fetch(h5, { body: { address: t43 } });
    let s11 = await this._privyInternal.refreshSession();
    return { user: s11.user, identity_token: s11.identity_token };
  }
  async link({ message: e40, signature: s11, walletClientType: l7, connectorType: n9 }) {
    await this._privyInternal.fetch(a11, { body: { message: e40, signature: s11, walletClientType: l7 ?? null, connectorType: n9 ?? null } });
    let r20 = await this._privyInternal.refreshSession();
    return { user: r20.user, identity_token: r20.identity_token };
  }
  async login({ mode: e40, message: t43, signature: l7, walletClientType: r20, connectorType: i22, opts: a20 }) {
    let d7 = await this._privyInternal.fetch(i4, { body: { signature: l7, message: t43, walletClientType: r20 ?? null, connectorType: i22 ?? null, mode: e40 } });
    await this._privyInternal.session.updateWithTokensResponse(d7);
    let o17 = await o13(this._embedded, d7, a20?.embedded);
    return this._privyInternal.callbacks?.setUser?.(o17.user), o17;
  }
  async fetchNonce({ address: e40 }) {
    let { nonce: t43 } = await this._privyInternal.fetch(t11, { body: { address: e40 } });
    return { nonce: t43 };
  }
  constructor(e40, t43) {
    this._privyInternal = e40, this._embedded = t43;
...
var e37 = class {
  async link(t43, e40, i22, s11) {
    await this._privyInternal.fetch(e7, { body: { message: t43, signature: e40, smart_wallet_type: i22, smart_wallet_version: s11 } });
    let a20 = await this._privyInternal.refreshSession();
    return { user: a20.user, identity_token: a20.identity_token };
  }
  async init(n9) {
    var e40;
    let { nonce: i22 } = await this._privyInternal.fetch(t10, { body: { address: n9.address } });
    return { nonce: i22, message: `${(e40 = { chainId: n9.chainId.toString().replace("eip155:", ""), address: n9.address, issuedAt: (/* @__PURE__ */ new Date()).toISOString(), statement: "By signing, you are proving you own this wallet and logging in. This does not initiate a transaction or cost any fees.", domain: "privy.io", uri: "https://auth.privy.io", nonce: i22 }).domain} wants you to sign in with your Ethereum account:
${e40.address}
...
URI: ${e40.uri}
Version: 1
Chain ID: ${e40.chainId}
Nonce: ${e40.nonce}
Issued At: ${e40.issuedAt}
Resources:
- https://privy.io` };
  }
  constructor(n9) {
    this._privyInternal = n9;
  }
};
...
var d4 = class {
  async logout(e40) {
    try {
      let e41 = await this._privyInternal.session.getRefreshToken() ?? void 0;
      await this._privyInternal.fetch(o3, { body: { refresh_token: e41 } });
    } catch (t43) {
      console.warn("Error destroying session");
    }
    await Promise.all([this._privyInternal.session.destroyLocalState({ reason: "logout" }), this._privyInternal.destroyClientAnalyticsId(), e40?.userId && this._mfa.clearMfa({ userId: e40.userId })]), this._privyInternal.callbacks?.setUser?.(null);
  }
  constructor(t43, d7, y2, u4, j) {
    this._privyInternal = t43, this._mfa = j, this.customProvider = new i15(this._privyInternal, d7), this.phone = new o16(this._privyInternal, d7), this.email = new a18(this._privyInternal, d7), this.oauth = new m2(this._privyInternal, d7, y2, u4), this.guest = new s9(this._privyInternal, d7), this.siwe = new r17(this._privyInternal, d7), this.siws = new r18(this._privyInternal, d7), this.smartWallet = new e37(this._privyInternal), this.passkey = new o15(this._privyInternal, d7), this.farcaster = new n7(this._privyInternal, d7), this.farcasterV2 = new i16(this._privyInternal, d7);
  }
};
...
var a19 = class {
  ...
  async getStatus(t43) {
    return await this._privyInternal.fetch(a, { query: { partnerUserId: t43 } });
  }
  constructor(t43) {
    this._privyInternal = t43;
  }
...
var i18 = class {
  async sign(t43) {
    return await this._privyInternal.fetch(o4, { body: t43 });
  }
  async getTransactionStatus({ transactionId: r20, useSandbox: i22 }) {
    let { url: n9, key: s11 } = d5[i22 ? "sandbox" : "prod"], e40 = await (0, import_fetch_retry2.default)(fetch, { retries: 3, retryDelay: 500 })(`${n9}/transactions/ext/${r20}?apiKey=${s11}`);
    if (!e40.ok) throw new o8({ error: `Failed to fetch transaction status for Transaction ${r20}`, code: "failed_to_fetch_moonpay_transaction_status", response: e40 });
    let c4 = await e40.json();
...
var i19 = class {
  constructor(i22) {
    this.moonpay = new i18(i22), this.coinbase = new a19(i22);
  }
};
...
var t42 = class {
  async sendCode(t43) {
    return await this._privyInternal.fetch(p4, { body: t43 });
  }
  constructor(r20) {
    this._privyInternal = r20;
  }
...
var i20 = class {
  ...
  async getAccessTokenInternal() {
    let t43 = await this.privyInternal.getAccessTokenInternal();
    if (!t43) throw new e24({ error: "Missing access token", code: "attempted_rpc_call_before_logged_in" });
    return t43;
  }
  async verifyMfa() {
  ...
  async initEnrollMfa(t43) {
    if (!this.proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_webview_not_loaded" });
    return await this.proxy.initEnrollMfa({ ...t43, accessToken: await this.getAccessTokenInternal() });
  }
  async submitEnrollMfa(t43) {
    if (!this.proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_webview_not_loaded" });
    let r20 = await this.proxy.submitEnrollMfa({ ...t43, accessToken: await this.getAccessTokenInternal() });
    return await this.privyInternal.refreshSession(), r20;
  }
  async unenrollMfa(t43) {
    if (!this.proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_webview_not_loaded" });
    let r20 = await this.proxy.unenrollMfa({ method: t43, accessToken: await this.getAccessTokenInternal() });
    return await this.privyInternal.refreshSession(), r20;
  }
  async clearMfa(t43) {
    if (!this.proxy) throw new e24({ error: "Embedded wallet proxy not initialized", code: "embedded_wallet_webview_not_loaded" });
    return await this.proxy.clearMfa(t43);
  }
  constructor(e40, i22) {
    this.proxy = i22, this.privyInternal = e40, this.sms = new t42(e40), this.passkey = new r19(e40);
  }
};
...
var e38 = class {
  ...
  async getICloudConfiguration(t43) {
    return this._privyInternal.fetch(h3, { body: { client_type: t43 } });
  }
  constructor(t43) {
    this._privyInternal = t43;
  }
...
var c3 = class {
  async generateURL(e40) {
    let r20 = i17(), c4 = c2(), _4 = await u2({ codeVerifier: r20, digest: this._crypto?.digest });
    return await Promise.all([this._storage.put(n8, r20), this._storage.put(t40, c4)]), this._privyInternal.fetch(a9, { body: { redirect_to: e40, code_challenge: _4, state_code: c4 } });
  }
  async authorize(t43, i22) {
    let [o17, a20] = await Promise.all([this._storage.get(n8), this._storage.get(t40)]);
    if (a20 !== i22) throw this._privyInternal.createAnalyticsEvent("possible_phishing_attempt", { flow: "recovery_oauth", storedStateCode: a20 ?? "", returnedStateCode: i22 ?? "" }), new e24({ code: "pkce_state_code_mismatch", error: "Unexpected auth flow. This may be a phishing attempt." });
    let c4 = await this._privyInternal.fetch(t9, { body: { authorization_code: t43, state_code: a20, code_verifier: o17 } });
    return await Promise.all([this._storage.del(n8), this._storage.del(t40)]), c4;
  }
  constructor(t43, e40, r20) {
    this._privyInternal = t43, this._storage = e40, this._crypto = r20;
  }
};

// node_modules/@privy-io/js-sdk-core/dist/esm/client/recovery/RecoveryApi.mjs
var i21 = class {
  async getRecoveryKeyMaterial(t43, e40) {
    return this._privyInternal.fetch(e5, { body: { chain_type: e40 }, params: { address: t43 } });
  }
  constructor(r20, i22, o17) {
    this._privyInternal = r20, this.auth = new c3(this._privyInternal, i22, o17), this.icloudAuth = new e38(this._privyInternal);
  }
};
...
var d6 = class {
  ...
  setMessagePoster(e40) {
    this.embeddedWallet.setMessagePoster(e40);
  }
  addOAuthTokensListener(e40) {
    return this._privyInternal.session.on("oauth_tokens_granted", e40), { unsubscribe: () => {
      this._privyInternal.session.removeListener("oauth_tokens_granted", e40);
    } };
  }
  setCallbacks(e40) {
    this._privyInternal.setCallbacks(e40);
  }
  getAccessToken() {
...
var e39 = class {
  async get(e40) {
    let t43 = localStorage.getItem(e40);
    return null === t43 ? void 0 : JSON.parse(t43);
  }
  put(e40, t43) {
    void 0 !== t43 ? localStorage.setItem(e40, JSON.stringify(t43)) : this.del(e40);
  }
  del(e40) {
    localStorage.removeItem(e40);
  }
  getKeys() {
    return Object.entries(localStorage).map((([e40]) => e40));
  }
};
...
function isPrivyConfigured(appId, clientId) {
  return Boolean(appId && clientId);
}
async function initPrivy(appId, clientId) {
...
function _extractEmail(user) {
  ...
  for (const a20 of accounts) {
    if (a20.type === "email" && a20.address) return a20.address;
  }
  return "";
}
